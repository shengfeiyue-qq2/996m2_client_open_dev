...
function ui.init(parent)
	...
	GUI:Layout_setBackGroundColorOpacity(CloseLayout, 76.0)
	GUI:setChineseName(CloseLayout, "申请列表_范围点击关闭")
	GUI:setTouchEnabled(CloseLayout, true)
	GUI:setTag(CloseLayout, -1.0)

	-- Create PMainUI
	local PMainUI = GUI:Layout_Create(parent, "PMainUI", 568.00, 320.00, 698.00, 418.00, false)
	...
	-- Create btmImage
	local btmImage = GUI:Image_Create(PMainUI, "btmImage", 348.00, 49.00, "res/public/bg_hhdb_01.jpg")
	GUI:setContentSize(btmImage, 660.0, 64.0)
	...
	GUI:setTag(btmImage, -1.0)

	-- Create line_bg
	local line_bg = GUI:Image_Create(PMainUI, "line_bg", 349.00, 360.00, "res/public/bg_yyxsz_01.png")
	GUI:setContentSize(line_bg, 660.0, 2.0)
	...
	GUI:setTag(line3, -1.0)

	-- Create Title1
	local Title1 = GUI:Text_Create(PMainUI, "Title1", 93.00, 380.00, 16.0, "#f2e7cf", [[玩家名字]])
	GUI:setChineseName(Title1, "申请列表_玩家名字_文本")
	GUI:setAnchorPoint(Title1, 0.50, 0.50)
	GUI:setTouchEnabled(Title1, false)
	GUI:setTag(Title1, -1.0)
	GUI:Text_enableOutline(Title1, "#000000", 1.0)

	-- Create Title2
	local Title2 = GUI:Text_Create(PMainUI, "Title2", 266.00, 380.00, 16.0, "#f2e7cf", [[等级]])
	GUI:setChineseName(Title2, "申请列表_等级_文本")
	GUI:setAnchorPoint(Title2, 0.50, 0.50)
	GUI:setTouchEnabled(Title2, false)
	GUI:setTag(Title2, -1.0)
	GUI:Text_enableOutline(Title2, "#000000", 1.0)

	-- Create Title3
	local Title3 = GUI:Text_Create(PMainUI, "Title3", 403.00, 380.00, 16.0, "#f2e7cf", [[职业]])
	GUI:setChineseName(Title3, "申请列表_职业_文本")
	GUI:setAnchorPoint(Title3, 0.50, 0.50)
	GUI:setTouchEnabled(Title3, false)
	GUI:setTag(Title3, -1.0)
	GUI:Text_enableOutline(Title3, "#000000", 1.0)

	-- Create Title4
	local Title4 = GUI:Text_Create(PMainUI, "Title4", 566.00, 380.00, 16.0, "#f2e7cf", [[操作]])
	GUI:setChineseName(Title4, "申请列表_操作_文本")
	GUI:setAnchorPoint(Title4, 0.50, 0.50)
	GUI:setTouchEnabled(Title4, false)
	GUI:setTag(Title4, -1.0)
	GUI:Text_enableOutline(Title4, "#000000", 1.0)

	-- Create CheckBox
	local CheckBox = GUI:CheckBox_Create(PMainUI, "CheckBox", 37.00, 49.00, "res/public/1900000654.png", "res/public/1900000655.png")
	GUI:setContentSize(CheckBox, 20.0, 22.0)
	...
	GUI:setTag(CheckBox, -1.0)

	-- Create Label
	local Label = GUI:Text_Create(CheckBox, "Label", 25.00, 0.00, 16.0, "#f2e7cf", [[自动同意入会申请]])
	GUI:setChineseName(Label, "申请列表_自动同意_文本")
	GUI:setTouchEnabled(Label, false)
	GUI:setTag(Label, -1.0)
	GUI:Text_enableOutline(Label, "#000000", 1.0)
	...
	GUI:setTag(TouchSize, -1.0)

	-- Create Input_bg
	local Input_bg = GUI:Image_Create(PMainUI, "Input_bg", 222.00, 47.00, "res/public/1900000668.png")
	GUI:setContentSize(Input_bg, 40.0, 20.0)
	...
	GUI:setTag(Input_bg, -1.0)

	-- Create Input
	local Input = GUI:TextInput_Create(PMainUI, "Input", 222.00, 50.00, 40.00, 20.00, 16.0)
	...
	GUI:setTag(Input, -1.0)

	-- Create text
	local text = GUI:Text_Create(PMainUI, "text", 247.00, 47.00, 16.0, "#f2e7cf", [[级以上]])
	GUI:setChineseName(text, "申请列表_级以上_文本")
	GUI:setAnchorPoint(text, 0.00, 0.50)
	GUI:setTouchEnabled(text, false)
	GUI:setTag(text, -1.0)
	GUI:Text_enableOutline(text, "#000000", 1.0)

	-- Create BtnAll
	local BtnAll = GUI:Button_Create(PMainUI, "BtnAll", 604.00, 48.00, "res/public/1900000660.png")
	GUI:Button_setTitleText(BtnAll, "一键通过")
	GUI:Button_setTitleColor(BtnAll, "#f8e6c6")
	GUI:Button_setTitleFontSize(BtnAll, 16.0)
	GUI:Button_titleEnableOutline(BtnAll, "#111111", 2.0)
	GUI:setChineseName(BtnAll, "申请列表_一键通过_按钮")
	GUI:setAnchorPoint(BtnAll, 0.50, 0.50)
	GUI:setTouchEnabled(BtnAll, true)
	GUI:setTag(BtnAll, -1.0)

	-- Create CloseButton
	local CloseButton = GUI:Button_Create(PMainUI, "CloseButton", 697.00, 416.00, "res/public/1900000510.png")
	...
	-- Create ListView
	local ListView = GUI:ListView_Create(PMainUI, "ListView", 22.00, 84.00, 653.00, 270.00, 1.0)
	GUI:ListView_setGravity(ListView, 5.0)
	GUI:setChineseName(ListView, "申请列表_所有申请列表")
	GUI:setTouchEnabled(ListView, true)
	GUI:setTag(ListView, -1.0)
end
return ui