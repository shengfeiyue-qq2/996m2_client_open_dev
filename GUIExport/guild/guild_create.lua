...
function ui.init(parent)
	...
	GUI:setContentSize(FrameBG, 548.0, 326.0)
	...
	GUI:setTag(bgTitle, -1.0)

	-- Create TitleText_1
	local TitleText_1 = GUI:Text_Create(PMainUI, "TitleText_1", 170.00, 240.00, 18.0, "#f2e7cf", [[行会名字]])
	GUI:setChineseName(TitleText_1, "创建行会_行会名字_文本")
	GUI:setAnchorPoint(TitleText_1, 0.50, 0.50)
	GUI:setTouchEnabled(TitleText_1, false)
	GUI:setTag(TitleText_1, -1.0)
	GUI:Text_enableOutline(TitleText_1, "#000000", 1.0)

	-- Create TitleText_2
	local TitleText_2 = GUI:Text_Create(PMainUI, "TitleText_2", 170.00, 150.00, 18.0, "#f2e7cf", [[需要道具]])
	...
	GUI:Image_setScale9Slice(Input_bg, 37, 35.0, 11, 15.0)
	GUI:setContentSize(Input_bg, 200.0, 30.0)
	...
	-- Create Input
	local Input = GUI:TextInput_Create(PMainUI, "Input", 320.00, 240.00, 200.00, 30.00, 20.0)
	...
	GUI:Button_setTitleFontSize(BtnCreate, 18.0)
	...
	-- Create Label
	local Label = GUI:Text_Create(CheckBox, "Label", 45.00, 5.00, 16.0, "#f2e7cf", [[自动同意入会申请]])
	...
	GUI:setContentSize(Input_level_bg, 40.0, 30.0)
	...
	GUI:setTag(Input_level_bg, -1.0)

	-- Create Input_level
	local Input_level = GUI:TextInput_Create(Input_level_bg, "Input_level", 0.00, 15.00, 40.00, 30.00, 16.0)
	...
	GUI:setTag(Input_level, -1.0)

	-- Create text
	local text = GUI:Text_Create(Input_level_bg, "text", 45.00, 15.00, 16.0, "#f2e7cf", [[级以上]])
