...
function ui.init(parent)
	-- Create Main_Target
	local Main_Target = GUI:Node_Create(parent, "Main_Target", 3.00, 190.00)
	GUI:setChineseName(Main_Target, "选中目标节点")
	GUI:setAnchorPoint(Main_Target, 0.50, 0.50)
	GUI:setTag(Main_Target, -1.0)

	-- Create Panel_1
	local Panel_1 = GUI:Layout_Create(Main_Target, "Panel_1", 3.00, 0.00, 210.00, 37.00, false)
	GUI:setChineseName(Panel_1, "选中目标_组合")
	GUI:setAnchorPoint(Panel_1, 0.00, 1.00)
	GUI:setTouchEnabled(Panel_1, true)
	GUI:setTag(Panel_1, -1.0)
	...
	GUI:setTag(nameBg, -1.0)

	-- Create nameText
	local nameText = GUI:Text_Create(Panel_1, "nameText", 105.00, 27.00, 16.0, "#ffffff", [[目标名称]])
	GUI:setChineseName(nameText, "选中目标_目标名称")
	GUI:setAnchorPoint(nameText, 0.50, 0.50)
	GUI:setTouchEnabled(nameText, false)
	GUI:setTag(nameText, -1.0)
	GUI:Text_enableOutline(nameText, "#111111", 1.0)
	...
	GUI:setTag(Image_hp, -1.0)

	-- Create hpLoadingBar
	local hpLoadingBar = GUI:LoadingBar_Create(Panel_1, "hpLoadingBar", 105.00, 8.00, "res/private/main/Target/1900012532.png", 0.0)
	GUI:LoadingBar_setPercent(hpLoadingBar, 50.0)
	...
	GUI:setTag(hpLoadingBar, -1.0)

	-- Create hpText
	local hpText = GUI:Text_Create(Panel_1, "hpText", 105.00, 9.00, 16.0, "#ffffff", [[100%]])
	GUI:setChineseName(hpText, "选中目标_目标Hp_%比")
	GUI:setAnchorPoint(hpText, 0.50, 0.50)
	GUI:setTouchEnabled(hpText, false)
	GUI:setTag(hpText, -1.0)
	GUI:Text_enableOutline(hpText, "#111111", 1.0)
	...
	GUI:Button_setTitleFontSize(cleanBtn, 10.0)
	GUI:Button_titleEnableOutline(cleanBtn, "#000000", 0.0)
	GUI:setChineseName(cleanBtn, "选中目标_关闭_按钮")
	GUI:setAnchorPoint(cleanBtn, 0.50, 0.50)
	GUI:setTouchEnabled(cleanBtn, true)
	GUI:setTag(cleanBtn, -1.0)
	...
	GUI:Button_setTitleFontSize(LockBtn, 10.0)
	GUI:Button_titleEnableOutline(LockBtn, "#000000", 0.0)
