...
function ui.init(parent)
	...
	-- Create TitleText
	local TitleText = GUI:Text_Create(FrameLayout, "TitleText", 36.00, 498.00, 20, "#d8c8ae", [[地图]])
	...
	-- Create Panel_map
	local Panel_map = GUI:Layout_Create(Panel_1, "Panel_map", 31.00, 41.00, 604.00, 442.00, true)
	...
	-- Create Image_clip
	local Image_clip = GUI:Image_Create(Panel_minimap, "Image_clip", 302.00, 221.00, "res/private/minimap/1900012104.png")
	GUI:setAnchorPoint(Image_clip, 0.50, 0.50)
	GUI:setTouchEnabled(Image_clip, false)
	GUI:setTag(Image_clip, 55)
	...
	-- Create Image_point
	local Image_point = GUI:Layout_Create(Panel_minimap, "Image_point", 100.00, 80.00, 100.00, 25.00, false)
	...
	local Image_mapNameBG = GUI:Image_Create(Panel_map, "Image_mapNameBG", 302.00, 440.00, "res/private/minimap/1900012107.png")
	GUI:setAnchorPoint(Image_mapNameBG, 0.50, 1.00)
	GUI:setTouchEnabled(Image_mapNameBG, false)
	GUI:setTag(Image_mapNameBG, -1)

	-- Create Text_mapName
	local Text_mapName = GUI:Text_Create(Panel_map, "Text_mapName", 302.00, 436.00, 16, "#ffffff", [[地图]])
