...
function ui.init(parent)
	...
	-- Create Panel_1
	local Panel_1 = GUI:Layout_Create(Scene, "Panel_1", 568.00, 320.00, 986.00, 536.00, false)
	...
	-- Create Panel_bg
	local Panel_bg = GUI:Layout_Create(Panel_1, "Panel_bg", 0.00, 268.00, 986.00, 536.00, false)
	...
	-- Create Image_bg
	local Image_bg = GUI:Image_Create(Panel_bg, "Image_bg", 493.00, 272.00, "res/private/rank_ui/rank_ui_mobile/1900020020.png")
	GUI:Image_setScale9Slice(Image_bg, 340, 340, 204, 204)
	GUI:setContentSize(Image_bg, 986, 536)
	...
	-- Create Panel_type
	local Panel_type = GUI:Layout_Create(Panel_1, "Panel_type", 169.00, 448.00, 185.00, 45.00, false)
	...
	local Panel_btnList = GUI:Layout_Create(Panel_1, "Panel_btnList", 80.00, 34.00, 125.00, 410.00, false)
	GUI:setChineseName(Panel_btnList, "排行榜_排行分类组合")
	GUI:setTouchEnabled(Panel_btnList, true)
	GUI:setTag(Panel_btnList, 24)

	-- Create ListView_btn
	local ListView_btn = GUI:ListView_Create(Panel_btnList, "ListView_btn", 0.00, 410.00, 125.00, 410.00, 1)
	...
	-- Create Image_title
	local Image_title = GUI:Image_Create(Panel_1, "Image_title", 490.00, 484.00, "res/private/rank_ui/rank_ui_mobile/1900020024.png")
	...
	local Panel_content = GUI:Layout_Create(Panel_1, "Panel_content", 212.00, 33.00, 700.00, 410.00, false)
	GUI:setTouchEnabled(Panel_content, false)
	GUI:setTag(Panel_content, -1)

	-- Create ListView_list
	local ListView_list = GUI:ListView_Create(Panel_content, "ListView_list", 8.00, 36.00, 450.00, 370.00, 1)
	...
	-- Create Node_model
	local Node_model = GUI:Node_Create(Panel_content, "Node_model", 564.00, 190.00)
	GUI:setChineseName(Node_model, "排行榜_模型_节点")
	GUI:setAnchorPoint(Node_model, 0.50, 0.50)
	GUI:setTag(Node_model, 58)

	-- Create Button_looks
	local Button_looks = GUI:Button_Create(Panel_content, "Button_looks", 568.00, 46.00, "res/public/1900000679.png")
	...
	-- Create Panel_myInfo
	local Panel_myInfo = GUI:Layout_Create(Panel_content, "Panel_myInfo", 8.00, 4.00, 450.00, 30.00, false)
	...
	-- Create Button_close
	local Button_close = GUI:Button_Create(Panel_1, "Button_close", 928.00, 445.00, "res/public/1900000510.png")
