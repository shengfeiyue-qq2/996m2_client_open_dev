...
function ui.init(parent)
	...
	GUI:setTag(Panel_itemTouch, 17.0)
	...
	GUI:setTag(Panel_lockStatus, 36.0)
	...
	GUI:setTag(Image_lock, 37.0)

	-- Create Text_name
	local Text_name = GUI:Text_Create(Panel_self, "Text_name", 102.00, 249.00, 18.0, "#ffffff", [[]])
	GUI:setChineseName(Text_name, "自己交易_昵称_文本")
	GUI:setAnchorPoint(Text_name, 0.50, 0.50)
	GUI:setTouchEnabled(Text_name, false)
	GUI:setTag(Text_name, 8.0)
	GUI:Text_enableOutline(Text_name, "#000000", 1.0)

	-- Create Text_gold
	local Text_gold = GUI:Text_Create(Panel_self, "Text_gold", 63.00, 37.00, 16.0, "#ffffff", [[0]])
	GUI:setChineseName(Text_gold, "自己交易_货币数量_文本")
	GUI:setAnchorPoint(Text_gold, 0.00, 0.50)
	GUI:setTouchEnabled(Text_gold, false)
	GUI:setTag(Text_gold, 9.0)
	...
	GUI:setContentSize(Image_locked, 340.0, 136.0)
	...
	GUI:Button_setScale9Slice(Button_trade, 15, 15.0, 12, 10.0)
	GUI:setContentSize(Button_trade, 82.0, 29.0)
	...
	GUI:Button_setTitleFontSize(Button_trade, 16.0)
	...
	GUI:Button_setScale9Slice(Button_lock, 15, 15.0, 12, 10.0)
	GUI:setContentSize(Button_lock, 82.0, 29.0)
	...
	GUI:Button_setTitleFontSize(Button_lock, 16.0)
	GUI:Button_titleEnableOutline(Button_lock, "#111111", 2.0)
	GUI:setChineseName(Button_lock, "自己交易_锁定_按钮")
	GUI:setAnchorPoint(Button_lock, 0.50, 0.50)
	GUI:setTouchEnabled(Button_lock, true)
	GUI:setTag(Button_lock, 40.0)
	...
	GUI:Button_setScale9Slice(btnClose, 8, 6.0, 12, 10.0)
	GUI:setContentSize(btnClose, 26.0, 42.0)
	...
	GUI:setTag(TouchSize, 15.0)
	GUI:setVisible(TouchSize, false)

	-- Create Panel_target
	local Panel_target = GUI:Layout_Create(PMainUI, "Panel_target", 5.00, 285.00, 400.00, 280.00, false)
	GUI:setChineseName(Panel_target, "对方交易组合")
	GUI:setTouchEnabled(Panel_target, true)
	GUI:setTag(Panel_target, 3.0)

	-- Create Image_bg
	local Image_bg = GUI:Image_Create(Panel_target, "Image_bg", 184.00, 138.00, "res/private/trade/trade_ui_mobile/bg_jiaoyidi_01.png")
	...
	-- Create Image_gold
	local Image_gold = GUI:Image_Create(Panel_target, "Image_gold", 25.00, 35.00, "res/private/bag_ui/bag_ui_mobile/1900015220.png")
	GUI:setChineseName(Image_gold, "对方交易_货币图标")
	GUI:setAnchorPoint(Image_gold, 0.50, 0.50)
	GUI:setTouchEnabled(Image_gold, false)
	GUI:setTag(Image_gold, 14.0)

	-- Create Image_forbid
	local Image_forbid = GUI:Image_Create(Panel_target, "Image_forbid", 25.00, 35.00, "res/private/trade/trade_ui_mobile/jinzhi.png")
	GUI:setChineseName(Image_forbid, "对方交易_禁止_图片")
	GUI:setAnchorPoint(Image_forbid, 0.50, 0.50)
	GUI:setTouchEnabled(Image_forbid, true)
	GUI:setTag(Image_forbid, 15.0)

	-- Create Panel_item
	local Panel_item = GUI:Layout_Create(Panel_target, "Panel_item", 19.00, 205.00, 340.00, 136.00, false)
	GUI:setChineseName(Panel_item, "对方交易_物品放置")
	GUI:setAnchorPoint(Panel_item, 0.00, 1.00)
	GUI:setTouchEnabled(Panel_item, true)
	GUI:setTag(Panel_item, 18.0)

	-- Create Panel_lockStatus
	local Panel_lockStatus = GUI:Layout_Create(Panel_target, "Panel_lockStatus", 19.00, 205.00, 340.00, 136.00, false)
	GUI:Layout_setBackGroundColorType(Panel_lockStatus, 1)
	GUI:Layout_setBackGroundColor(Panel_lockStatus, "#000000")
	GUI:Layout_setBackGroundColorOpacity(Panel_lockStatus, 102.0)
	...
	GUI:setAnchorPoint(Panel_lockStatus, 0.00, 1.00)
	GUI:setTouchEnabled(Panel_lockStatus, false)
	GUI:setTag(Panel_lockStatus, 38.0)
	GUI:setVisible(Panel_lockStatus, false)
	...
	GUI:setAnchorPoint(Image_lock, 0.50, 0.50)
	GUI:setTouchEnabled(Image_lock, false)
	GUI:setTag(Image_lock, 39.0)

	-- Create Text_name
	local Text_name = GUI:Text_Create(Panel_target, "Text_name", 100.00, 256.00, 18.0, "#ffffff", [[]])
	GUI:setChineseName(Text_name, "对方交易_昵称_文本")
	GUI:setAnchorPoint(Text_name, 0.50, 0.50)
	GUI:setTouchEnabled(Text_name, false)
	GUI:setTag(Text_name, 7.0)
	GUI:Text_enableOutline(Text_name, "#000000", 1.0)

	-- Create Text_gold
	local Text_gold = GUI:Text_Create(Panel_target, "Text_gold", 64.00, 43.00, 16.0, "#ffffff", [[0]])
	GUI:setChineseName(Text_gold, "对方交易_货币数量_文本")
	GUI:setAnchorPoint(Text_gold, 0.00, 0.50)
	GUI:setTouchEnabled(Text_gold, false)
	GUI:setTag(Text_gold, 10.0)
	GUI:Text_enableOutline(Text_gold, "#111111", 1.0)

	-- Create Text_locked
	local Text_locked = GUI:Text_Create(Panel_target, "Text_locked", 282.00, 45.00, 16.0, "#ffe400", [[已锁定]])
	GUI:setChineseName(Text_locked, "对方交易_已锁定_文本")
	GUI:setAnchorPoint(Text_locked, 0.50, 0.50)
	GUI:setTouchEnabled(Text_locked, false)
	GUI:setTag(Text_locked, 22.0)
	GUI:setVisible(Text_locked, false)
	GUI:Text_enableOutline(Text_locked, "#111111", 2.0)

	-- Create Image_locked
	local Image_locked = GUI:Image_Create(Panel_target, "Image_locked", 19.00, 205.00, "res/public/1900000678_3.png")
	GUI:setContentSize(Image_locked, 340.0, 136.0)
	GUI:setIgnoreContentAdaptWithSize(Image_locked, false)
	GUI:setChineseName(Image_locked, "对方交易_锁定_图片")
	GUI:setAnchorPoint(Image_locked, 0.00, 1.00)
	GUI:setTouchEnabled(Image_locked, false)
	GUI:setTag(Image_locked, 25.0)
	GUI:setVisible(Image_locked, false)

	-- Create btnClose
	local btnClose = GUI:Button_Create(Panel_target, "btnClose", 383.00, 196.00, "res/public/1900000510.png")
	GUI:Button_loadTexturePressed(btnClose, "res/public/1900000511.png")
	GUI:Button_setScale9Slice(btnClose, 8, 6.0, 12, 10.0)
	GUI:setContentSize(btnClose, 26.0, 42.0)
	GUI:setIgnoreContentAdaptWithSize(btnClose, false)
