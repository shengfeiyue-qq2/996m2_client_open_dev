...
local SetProtectCfg = {
    ...
    -- { id = 3038, content = "红名使用", default = 0, type = 2 }
}
...
local SetHeroCfg = {
    ...
    [2] = {
        ...
        { id = 3050, content = "复活戒指回城保护英雄",    default = 0 }
    }
}

-- 挂机配置
local HangCfg = {
    { id = 3039, content = "被玩家攻击时",             default = 0, default3 = 0, operates = {[1] = "不处理", [2] = "反击", [3] = "逃跑"}},   -- default1: 格数
    { id = 3040, content = "身边%s格有怪时不捡物",      default = 0, default1 = 0},     -- default1: 格数
    { id = 3041, content = "受到BB攻击时先打主人",      default = 0 },
    { id = 3042, content = "不抢别人归属怪物",          default = 0 },
    { id = 3047, default = {}},
    { id = 3043, content = "寻路%s秒没有怪物时使用",    default = 0, default1 = 0, default3 = 0, indexs = ""},               -- default1: 秒数  default3: 物品index 
    { id = 3044, content = "周围%s格有%s个敌人使用",    default = 0, default1 = 0, default2 = 0, default3 = 0, indexs = ""}, -- default1: 格数  default2: 数量   default3: 物品index
    { id = 3045, content = "周围%s格有%s个红名使用",    default = 0, default1 = 0, default2 = 0, default3 = 0, indexs = ""}, -- default1: 格数  default2: 数量   default3: 物品index
    { id = 3046, content = "周围%s格有敌人时主动攻击",   default = 0, default1 = 8}     -- default1: 格数
}
...
GUIShare.SetCfg = {
    SetBaseCfg = SetBaseCfg, SetProtectCfg = SetProtectCfg, SetBattleCfg = SetBattleCfg, HangCfg = HangCfg, SetHeroCfg = SetHeroCfg, SetOtherCfg = SetOtherCfg
}
------------------------------------------------------------------------------------------------------------------------------------
-- 内挂技能优先级设置
local setOtherSkilllPriority = function (setID, skillID)
    if  SL:CheckSet(setID) == 1 then
        SL:ResetSkillPriority(skillID)
    else
        SL:SetSkillPriority(skillID, 0)
    end
end

-- 系统自带技能 ---------------------------------------
-- 查找技能
GUIShare.FindSkills = function (skilltype)
    local allSkills = SL:FindAllSkillsByJob()
    local items = {}

    for i = 1, #allSkills do
        local skill = allSkills[i]
        if skill and skill.MagicID ~= 0 and skill.skilltype == skilltype then
...
-- 单体、群体、加血治疗、召唤 优先级
local setSpecialSkillPriority = function (skills, setID)
    local values     = SL:GetSettingValue(setID) or {}
    local enabled    = values[1] == 1
    local skillID    = -1
    local condition  = nil
    
    if setID == 25 then
        skillID      = values[3]
        local minHp  = values[2] or 0
        condition    = SL:GetMetaValue("ROLE_HP_PER") < minHp
    else
        skillID      = values[2]        
    end

    for _, skill in pairs(skills) do
        local magicID = skill.MagicID
        if magicID == skillID and not enabled and not condition then
            SL:SetSkillPriority(magicID, 0)
        else
            SL:ResetSkillPriority(magicID)
        end
    end
end

-- 内挂设置技能
local checkSetSkills = function (setID)
    if setID == 47 then
        setSpecialSkillPriority(GUIShare.FindSkills(1), setID)         -- 单体技能
    elseif setID == 21 then
        setSpecialSkillPriority(GUIShare.FindSkills(2), setID)         -- 群体技能
    elseif setID == 25 then
        setSpecialSkillPriority(GUIShare.FindSkills(3), setID)         -- 治疗加血
    elseif setID == 26 then
        setSpecialSkillPriority(GUIShare.FindSkills(4), setID)         -- 自动召唤
    end
end

GUIShare.skillCalcPriority = {
    [21]  = function() checkSetSkills(21) end,                                  -- 群怪技能
    [47]  = function() checkSetSkills(47) end,                                  -- 单体技能
    [25]  = function() checkSetSkills(25) end,                                  -- 自动加血
    [26]  = function() checkSetSkills(26) end,                                  -- 自动召唤
    [53]  = function() setOtherSkilllPriority(53, 66) end,                      -- 自动开天
    [54]  = function() setOtherSkilllPriority(54, 56) end,                      -- 自动逐日
    [16]  = function() setOtherSkilllPriority(16, 26) end,                      -- 自动烈火
    [19]  = function() setOtherSkilllPriority(19, 31) end,                      -- 自动魔法盾
    [20]  = function() setOtherSkilllPriority(20, 22) end,                      -- 自动火墙
    [22]  = function() setOtherSkilllPriority(22, 6)  end,                      -- 自动施毒术
    [23]  = function() setOtherSkilllPriority(23, 14) end,                      -- 自动幽灵盾
    [24]  = function() setOtherSkilllPriority(24, 15) end,                      -- 自动神圣战甲术
    [59]  = function() setOtherSkilllPriority(59, 50) end,                      -- 自动无极真气
    [111] = function() setOtherSkilllPriority(111, 111) end                     -- 嗜血术 
}


-- 自定义技能 ---------------------------------------
GUIShare.CustomSetSkills = {}

-- 自动自定义技能(setID=skillID, groupID=skillID)
local checkCustom = function (skillID)
    if not SL:IsCustomSkill(skillID) then
        return false
    end
    setOtherSkilllPriority(skillID, skillID)
end

GUIShare.SetCustomSetSkill = function (skillID)
    GUIShare.CustomSetSkills = GUIShare.CustomSetSkills or {}
    GUIShare.CustomSetSkills[skillID] = true
end

GUIShare.InitCustomSetSkill = function (pSkillID)
    if pSkillID then
        for skillID, v in pairs(GUIShare.CustomSetSkills or {}) do
            if skillID and skillID == pSkillID and v then
                checkCustom(skillID)
            end
        end
    else
        for skillID, v in pairs(GUIShare.CustomSetSkills or {}) do
            if skillID and v then
                checkCustom(skillID)
            end
        end
    end
...
GUIShare.FindEatCDByIndex = function (index)
    local CDs = {
        ...
        -- [3038] = {CD = 1000, minCD = 1000, maxCD = 9999},
    }        
...
-- 与身上装备对比逻辑
GUIShare.CompareEquipOnBody = function (itemData, from)
    local ItemFrom  = SL:GetItemForm()
    local IsBag     = from and from == ItemFrom.BAG
    local IsHeroBag = from and from == ItemFrom.HERO_BAG

    local flag = SL:GetMetaValue("EQUIP_COMPARISON", itemData.Index)
    if flag == -1 then                      -- 拾取 人物和背包 都不对比
        return false
    elseif flag == -2 and IsBag then        -- 拾取 进人物背包 不对比
        return false
    elseif flag == -3 and IsHeroBag then    -- 拾取 进英雄背包 不对比
        return false
    elseif flag == -5 then                  -- 拾取和身上穿戴 不与任何作对比
        return false
    end

    -- 检测装备
    local posList = IsHeroBag and SL:GetHeroEquipPosByStdModeList(itemData.StdMode) or SL:GetEquipPosByStdModeList(itemData.StdMode)
    ...
    for i = 1, n do
        local pos = posList[i]
        if pos then
            local myEquip = IsHeroBag and SL:GetMetaValue("H.EQUIP_DATA_BY_POS", pos) or SL:GetMetaValue("M.EQUIP_DATA_BY_POS", pos)
            if myEquip then
                if tonumber(myEquip.comparison) ~= -4 and tonumber(myEquip.comparison) ~= -5 then -- 身上穿戴 不与任何作对比
                    return true
                end
            else
                return true
...
-- 获取当前StdMode最小装备位
GUIShare.GetMinPowerPosByStdMode = function (StdMode, param, isHero)
    local StdMode = StdMode or 0
    local onEquipMinPower = 0
    local minPowerPos = -1
    local hasEquip = true
    local pos = isHero and SL:GetHeroEquipPosByStdModeList(StdMode) or SL:GetEquipPosByStdModeList(StdMode)
    if not pos or next(pos) == nil then
        return minPowerPos, onEquipMinPower
    end
    for k,v in ipairs(pos) do
        local equipData = isHero and SL:GetHeroEquipDataByPos(v) or SL:GetEquipDataByPos(v)
        ...
        local equipPower = SL:GetMetaValue("EQUIP_POWER", equipData, param, isHero)
        if minPowerPos == -1 or onEquipMinPower > equipPower then
...
-- 检测装备是否超负重了
GUIShare.CheckEquipWeight = function(srcItem, pos, isHero)
    if not srcItem or not next(srcItem) or not pos or pos < 0 then
        return false
    end

    -- 角色 Weight 值
    local curType = BaseAttTypeTable.Wear_Weight
    local maxType = BaseAttTypeTable.Max_Wear_Weight
    if EquipHandWeightType[srcItem.StdMode] then
        curType = BaseAttTypeTable.Hand_Weight
        maxType = BaseAttTypeTable.Max_Hand_Weight
    end
    local playMaxWeight = SL:GetRoleAttByType(maxType) or 0
    local playCurWeight = SL:GetRoleAttByType(curType) or 0

    -- 身上装备 Weight
    local dstItem = isHero and SL:GetHeroEquipDataByPos(pos) or SL:GetEquipDataByPos(pos)
    local dstItemWeight = dstItem and dstItem.Weight or 0

    -- 目标装备 Weight
    local srcItemWeight = srcItem.Weight

    local totalWeight = srcItemWeight - dstItemWeight + playCurWeight

    return  totalWeight <= playMaxWeight
end
------------------------------------------------------------------------------------------------------------------------------------
-- 背包格子配置
GUIShare.BagGridSet = {
...
-- 技能不释放列表
GUIShare.NotLaunchSkills = {}
------------------------------------------------------------------------------------------------------------------------------------
-- 音效

-- ******* 注意： SL:PlayAudio(index, type) 除了 SND_TYPE_UI（UI音效）, 其他音效ID 均不用传值 *********
GUIShare.SND_TYPE_SKILL_START                = 1     -- 释放技能
GUIShare.SND_TYPE_SKILL_FIRE                 = 2     -- 技能飞行
GUIShare.SND_TYPE_SKILL_EXPLOSION            = 3     -- 技能击中爆炸
GUIShare.SND_TYPE_SKILL_ATTACK               = 4     -- 普攻
GUIShare.SND_TYPE_MONEY                      = 5     -- 货币变化
GUIShare.SND_TYPE_LOGIN_DOOR                 = 6     -- 登陆界面开门
GUIShare.SND_TYPE_LOGIN_SELECTONE            = 7     -- 登陆界面选择角色  
GUIShare.SND_TYPE_FLY_OUT                    = 8     -- 飞出视野
GUIShare.SND_TYPE_FLY_IN                     = 9     -- 飞入视野
GUIShare.SND_TYPE_STUCK_WEAPON               = 10    -- 受击 攻击方武器的声音
GUIShare.SND_TYPE_STUCK                      = 11    -- 受击 碰撞的声音
GUIShare.SND_TYPE_SCREAM                     = 12    -- 受击 受害者发出的声音
GUIShare.SND_TYPE_PLAYER_DIE                 = 13    -- 玩家死亡
GUIShare.SND_TYPE_CLICK_ITEM                 = 15    -- 点击道具
GUIShare.SND_TYPE_USE_ITEM                   = 16    -- 使用道具
GUIShare.SND_TYPE_ACT_MOVE                   = 17    -- 主角移动
GUIShare.SND_TYPE_MON_BORN                   = 18    -- 怪物出生
GUIShare.SND_TYPE_MON_MOVE                   = 19    -- 怪物移动
GUIShare.SND_TYPE_MON_ATTACK                 = 20    -- 怪物攻击
GUIShare.SND_TYPE_MON_DIE                    = 21    -- 怪物死亡
GUIShare.SND_TYPE_UI                         = 22    -- UI音效
GUIShare.SND_TYPE_UPGRADE                    = 24    -- 升级
GUIShare.SND_TYPE_MINING                     = 25    -- 挖矿
GUIShare.SND_TYPE_MINING_DROP                = 26    -- 挖矿起灰
GUIShare.SND_TYPE_LAYER_CLICK                = 27    -- 界面点击 
GUIShare.SND_TYPE_LAYER_CLOSE                = 28    -- 界面点×关闭
GUIShare.SND_TYPE_FLY_IN_HERO                = 30    -- 英雄出生音效
GUIShare.SND_TYPE_FLY_OUT_HERO               = 31    -- 英雄收回音效

-- BGM
GUIShare.SND_TYPE_BGM_LOGIN                  = 101   -- 登陆界面
GUIShare.SND_TYPE_BGM_SELECT                 = 102   -- 选角界面
GUIShare.SND_TYPE_BGM_DIE                    = 103   -- 死亡
GUIShare.SND_TYPE_BGM_MAP                    = 104   -- 地图
------------------------------------------------------------------------------------------------------------------------------------
...
--     time        气泡消失时间
--     callback    气泡点击回调
--     timeOverCB  气泡消失执行回调(params是系统本身的气泡数据)
-- }
GUIShare.BubbleExCfg = { }

-- 自动战斗群攻技能检测数量
GUIShare.GroupAttackSkillCheckNums = 1

-- key: 1: 数值; 2: 百分比(value: 0~100)
GUIShare.DuraTipsCfg = {
    key = 1,
    value = 1000
}
...
-- 当前模式是否显示
GUIShare.IsShowCurMode = function(mode)
    return humanModeControl[mode]
end

-- 获取模式列表
GUIShare.GetHumanModeDatas = function ()
    return SL:GetMetaValue("KFSTATE") and {0, 1, 4, 5, 6, 3, 2, 7, 8, 9} or {0, 1, 4, 5, 6, 3, 2, 7, 8}
end

-- 获取阵营文本
GUIShare.GetHumanModeStr = function (mode)
    return humanModeStr[mode] or humanModeStr[0]
end

------------------------------------------------------------------------------------------------------------------------------------
-- 处理掉落物名字
GUIShare.FixDropItemName = function (name)
    return name or "ERROR"
end
------------------------------------------------------------------------------------------------------------------------------------
-- 黑夜模式默认基础格子数
GUIShare.DarkModeBaseGridCount = 0

-- 场景相机偏移
GUIShare.SceneCameraOffset = {x = 0, y = 0}

-- 更新CostItemCell
GUIShare.CostItemCellUpdate = function (ui, data, extra)
    local sItems = string.split(data, "#")
    local itemID = tonumber(sItems[1]) or 1
    local count  = tonumber(sItems[2]) or 1

    extra = extra or {}

    local abbr      = extra.abbr                    -- 数量是否简称
    local pointBit  = extra.pointBit                -- 数量保留几位小数（abbr必须为true）
    local showTip   = not extra.noShowTip           -- 是否显示描述文本
    local titleText = extra.titleText or "消耗:"    -- 描述文本
    local lineData  = extra.cutLineData             -- 下划线
    local enbaled   = extra.enbaled                 -- 点击查看tips

    local uiTips, uiIcon, uiCount = ui["Node_tips"], ui["Node_icon"], ui["Node_count"]

    -- 父节点
    local uiParent = GUI:getParent(uiIcon)

    -- 道具缩放大小
    local scale = 0.5

    -- 道具
    GUI:removeAllChildren(uiIcon)
    local goodsItem = GUI:ItemShow_Create(uiIcon, "icon", 0, 0, {index = itemID, mouseCheckTimes = extra.mouseCheckTimes})
    GUI:setScale(goodsItem, scale)
    GUI:setAnchorPoint(goodsItem, 0.5, 0.5)

    -- 货币
    local itemCount  = SL:GetItemNumberByIndex(itemID, true)
    local color      = itemCount >= count and "#28EF01" or "#FF0500"
    local numStr     = abbr and SL:GetSimpleNumber(count, pointBit) or count
    local contentStr = string.format("<font color='%s'>%s</font>", color, numStr)

    GUI:removeAllChildren(uiCount)
    local richText = GUI:RichText_Create(uiCount, "Label", 0, 0, contentStr, 400, 16, "#FFFFFF")
    GUI:setAnchorPoint(richText, 0, 0.5)

    -- 描述
    GUI:removeAllChildren(uiTips)
    if showTip then
        local tip = GUI:Text_Create(uiTips, "tip", 0, 0, 16, "#FFFFFF", titleText)
        GUI:setAnchorPoint(tip, 0, 0.5)

        local tips_width = GUI:getContentSize(tip).width
        local icon_width = GUI:getContentSize(goodsItem).width * scale

        local dis = 1
        local wid = tips_width + icon_width + GUI:getContentSize(richText).width + dis * 2

        local contentSize = {
            width = wid, height = GUI:getContentSize(uiParent).height
        }
        GUI:setContentSize(uiParent, contentSize)
    
        GUI:setPositionX(uiTips, 0)
        GUI:setPositionX(uiIcon, tips_width + icon_width / 2 + dis)
        GUI:setPositionX(uiCount, tips_width + icon_width + 3 + dis)
    
        GUI:setContentSize(ui, contentSize)
        GUI:setPosition(uiParent, contentSize.width / 2, contentSize.height / 2)
    end

    -- 下划线
    if lineData then
        local drawLine = GUI:CreateDrawLine(
            lineData.size or 3, 
            lineData.beginPos or cc.p(-3, 0), 
            lineData.endPos or cc.p(GUI:getContentSize(richText).width + 3, 0), 
            lineData.color or cc.c4f(1, 0, 0, 0.5))

        local linePos = lineData.pos or cc.p(0, 0)
        drawLine:setPosition(linePos)
        GUI:addChild(uiCount, drawLine)
    end

    -- 点击查看tips
    if enbaled then
        GUI:setTouchEnabled(uiParent, true)
        GUI:RemoveAllWidgetClickEventListener(uiParent)
        GUI:addOnClickEvent(uiParent, function () SL:OpenItemTips({Index = itemID, pos = GUI:getTouchEndPosition(uiParent)}) end)
    else
        GUI:setTouchEnabled(uiParent, false)
    end
end
...
GUIShare.CheckTargetDigAble = function (targetID)
    -- 必须是死亡的
    ...
    -- 人形怪可挖（没有归属）
    if SL:GetMetaValue("ACTOR_IS_HUMAN", targetID) and not SL:GetMetaValue("ACTOR_HAVE_MASTER", targetID) then
        return true
    end

    -- 怪物
    if not SL:GetMetaValue("ACTOR_IS_MONSTER", targetID) then
        return false
    end

    local raceServer = SL:GetMetaValue("ACTOR_RACE_SERVER", targetID)
    if not DIG_RACE_SERVER_LIST[raceServer] then
        return false
    end

    -- 配置不可以挖
    local typeIndex = SL:GetMetaValue("ACTOR_TYPE_INDEX", targetID)
    local data      = SL:GetMetaValue("GAME_DATA", "noDigMonsters") or ""
