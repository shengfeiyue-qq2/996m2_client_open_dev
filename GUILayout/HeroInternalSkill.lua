...
function HeroInternalSkill.UpdateSkillCells()
    ...
    for i, v in ipairs(items) do
        ...
        local cell = HeroInternalSkill.CreateSkillCell(HeroInternalSkill._ui.ListView_cells, skillID, skillType)
        HeroInternalSkill._cells[string.format("%s_%s", skillID, skillType)] = cell
    end

    HeroInternalSkill.RefreshSkillCells()
end
...
function HeroInternalSkill.CreateSkillCell(parent, skillID, skillType)
    HeroInternalSkill._index = HeroInternalSkill._index + 1
    local cellW = HeroInternalSkill._isWIN32 and 244 or 310
    local cellH = HeroInternalSkill._isWIN32 and 50 or 70
    local widget = GUI:Widget_Create(parent, "SkillItems_" .. HeroInternalSkill._index, 0, 0, cellW, cellH)
    if HeroInternalSkill._isWIN32 then
        GUI:LoadExport(widget, "internal_hero/skill_cell_win32")
    else
        GUI:LoadExport(widget, "internal_hero/skill_cell")
    end
