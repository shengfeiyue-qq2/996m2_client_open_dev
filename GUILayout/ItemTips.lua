...
------------  装备tips  ------------------------------------------------
function ItemTips.CreateEquipPanel(data, itemData, isWear)
    ...
    local ListView = GUI:ListView_Create(ListBg, "ListView", 14, 10, 0, 0, 1)
    GUI:ListView_setItemsMargin(ListView, 0)
    GUI:setTouchEnabled(ListView, false)

    -- 道具名字
    local nameStr = SL:GetMetaValue("ITEM_NAME_COLOR_BY_DATA", itemData)
    local r_name = GUI:RichText_Create(ListView, "r_name", 0, 0, nameStr, richWidth, _TextSize, nil, nil, nil, fontPath)
    ...
    if isBindUser then
        local r_bindUser = GUI:RichText_Create(ListView, "r_bindUser", 0, 0, string.format("已绑定[%s]", itemData.BindInfo), richWidth, _TextSize, "#FFFFFF", nil, nil, fontPath)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    if isBind then
        local r_bind = GUI:RichText_Create(ListView, "r_bind", 0, 0, "已绑定", richWidth, _TextSize, "#FFFFFF", nil, nil, fontPath)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    -- 套装属性
    local suitArr = SL:Split(itemData.Suit or itemData.suitid or "", "#")
    ...
    if ListSuit then
        ...
        GUI:setContentSize(ListSuitBg, {width = listSuitSize.width + 28, height = listSuitSize.height + 20})
    end
    ...
    -- 倒计时
    local endTime = ItemTips.GetEndTime(itemData)
    if endTime then
        ...
        if day < 1 then
            local function showTime()
                ...
                GUI:Text_setString(Text_time, getTimeStr(time))
            end
    ...
    -- 物品来源
    local itmsrc = itemData.ExtendInfo and itemData.ExtendInfo.ItmSrc
    local source_tab = {"地图", "出处", "角色", "时间"}
    if itmsrc then
        local item_source_src = string.format("<font color='%s'>%s</font>", SL:GetColorByID(154), "[物品来源]：") .. "<br>"
        for id, value in ipairs(itmsrc) do 
            if value ~= "" and id < 5 then
                if id == 4 then
                    value = os.date("%Y-%m-%d %H:%M:%S", value)
                end
                local str = string.format("<font color='%s'>%s</font>", SL:GetColorByID(251), source_tab[id] .. ":" .. value)
                item_source_src = item_source_src ..str .. "<br>"
            end
        end
        local item_source = GUI:RichText_Create(ListView, "item_source", 0, 0, item_source_src, ww + 120,_TextSize, 1038)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
        size = GUI:getContentSize(item_source)
        ww = math.max(ww, size.width)
        hh = hh + size.height + _DefaultSpace
    end

    local ListBgWidth = ww + 28
...
function ItemTips.GetSuitStr(suitID, itemData)
    ...
    local function CheckIsWearEquipPos(pos)
        ...
        local suits = eData and (eData.Suit or eData.suitid or "") or ""
        if string.len(suits) > 0 then 
            local suitArr = string.split(suits, "#")
...
-- 道具时限-剩余时间
function ItemTips.GetEndTime(itemData)
