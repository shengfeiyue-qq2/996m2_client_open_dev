...
function ItemTips.CreateEquipPanel(data, itemData, isWear, isCompare)
    ...
    if typeStr and string.len(typeStr) > 0 then
        ...
        local r_type = GUI:RichText_Create(Icon, "r_type", wx, wy, typeStr, richWidth, _TextSize, 1038, nil, nil, fontPath)
    ...
    if needStr then
        ...
        local r_need = GUI:RichText_Create(Icon, "r_need", wx, wy, needStr, richWidth, _TextSize, 1038, nil, nil, fontPath)
    ...
    if jobStr then
        ...
        local wy = IconBgSz.height * 0.7
        local r_job = GUI:RichText_Create(Icon, "r_job", wx, wy, jobStr, richWidth, _TextSize, 1038, nil, nil, fontPath)
    ...
    if string.len(baseAttStr) > 0 then
        local r_att1 = GUI:RichText_Create(ListView, "r_att1", 0, 0, baseAttStr, richWidth, _TextSize, 1038, nil, nil, fontPath)
        ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    for k,suitID in ipairs(suitArr) do
        ...
        if suitID > 0 then
            ...
            if ListSuit then
                local r_suit = GUI:RichText_Create(ListSuit, "r_suit"..k, 0, 0, suitStr, richWidth, _TextSize, 1038, nil, nil, fontPath)
                ...
                local r_suit = GUI:RichText_Create(ListView, "r_suit"..k, 0, 0, suitStr, richWidth, _TextSize, 1038, nil, nil, fontPath)
                ItemTips.CreateIntervalPanel(ListView, _DefaultSpace, true)
    ...
    if desc and string.len(desc) > 0 then
        local r_desc = GUI:RichText_Create(ListView, "r_desc", 0, 0, desc, ww + 10, _TextSize, SL:GetColorByID(1038), nil, nil, fontPath)
...
function ItemTips.GetSuitStr(suitID, itemData)
    ...
    for _,nameStr in ipairs(nameStrs) do
        if nameStr and string.len(nameStr) > 0 then
            ...
            local color, str = ParseColor(showArr[1].."|"..nameStr, isWear and 2 or 1)
    ...
    if descStr and string.len(descStr) > 0 then
        local color = ParseColor(descStr, wearCount >= suitCount and 2 or 1)

        local descs = SL:Split(SL:Split(descStr,"|")[2], "<br>")
        for _,dsc in ipairs(descs) do
            if dsc and string.len(dsc) > 0 then
                suitStr = suitStr .. string.format("<font colorStyleID='%s'>%s</font><br>", color, dsc)
            end
        end
    end
...
function ItemTips.CreateItemPanel(data, itemData)
    ...
    local aStr = ItemTips.GetOtherAttrStr(itemData)
    if aStr then
        local r_mode = GUI:RichText_Create(ListView, "r_mode", wx, wy, aStr, richWidth, _TextSize, 1038, nil, nil, fontPath)
        GUI:setAnchorPoint(r_mode, 0, 0.5)
