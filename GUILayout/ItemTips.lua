...
function ItemTips.GetModeStr(itemData)
    local function GetDuraStr(dura, maxdura)
        return string.format("%s/%s", math.round(dura / 1000), math.round(maxdura / 1000))
    end
    local str = nil
    ...
    elseif EquipMapByStdMode[itemData.StdMode] or showLasting[itemData.StdMode] then
        str = string.format("持久：%s", GetDuraStr(itemData.Dura, itemData.DuraMax))
        checkDura = itemData.Dura / 1000
    elseif itemData.StdMode == 40 then --肉
        str = string.format("品质：%s", GetDuraStr(itemData.Dura, itemData.DuraMax))
...
function ItemTips.GetAttStr(itemData, diff)
    ...
    -- 基础属性
    local attList           = GUIShare.ParseItemBaseAtt(itemData.attribute)
    ...
    --附加的元素属性
    local yuansuAtt = GUIShare.GetExAttList(yuansuValues)
    if yuansuAtt and next(yuansuAtt) then
        attList = GUIShare.CombineAttList(attList, yuansuAtt)
    end

    -- 极品属性
    local exAtt = GUIShare.GetExAttList(jipinValues)
    local exAttShow = GUIShare.GetAttDataShow(exAtt, true, true)
    -- 合并极品属性
    ...
    local attUpList = {}
    if diff then
        table.insert(ItemTips._equipAttList, GUIShare.GetAttDataShow(attList, nil, true))
    elseif next(ItemTips._equipAttList) then
        local curAttList = GUIShare.GetAttDataShow(attList, nil, true)
    ...
    -- 属性显示队列
    local stringAtt = GUIShare.GetAttDataShow(attList, nil, true)
    --把基础属性和元素属性分开
    local basicAttrShow = {}
        ...
        if id > 0 then
            ...
            elseif id > 1 then
                ...
                if gemData then
                    ...
                    local attList = GUIShare.ParseItemBaseAtt(gemData.attribute)
                    local attShow = GUIShare.GetAttShowOrder(attList, nil, true)
                    local attStr = ""
...
function ItemTips.GetSuitPanel(suitConfig, itemData, idx)
    ...
    if nameColor and meetCount >= suitCount then
    ...
    local descs = GUIShare.GetParseItemDesc(suitNameStr .. equipNameStr)
    ...
    local attrDescs = GUIShare.GetParseItemDesc(suitConfig.equipAttStr)
...
function ItemTips.AddFrameEffect(parent, itemDescs)
    ...
    if frameTopDesc and next(frameTopDesc) then
        ...
        for i, children in ipairs(frameChilds) do
            if children then
                GUI:setPositionY(children, GUI:getPositionY(children) + tipsPanelSz.height)
                animScale(children)
            end
        end
    end
...
function ItemTips.CreateEquipPanel(data, itemData, isWear, panelInsertIndex)
    ...
    local itemDescs = GUIShare.GetParseItemDesc(itemData.Desc)
    -- 顶部描述
...
function ItemTips.CreateItemPanel(data, itemData)
    ...
    -- 道具描述
    local itemStr = GUIShare.GetItemAttDesc(itemData) or {}
    local attrStr = ""
