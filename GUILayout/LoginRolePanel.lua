...
function LoginRolePanel.main()
    ...
    LoginRolePanel.InitEvent()
    LoginRolePanel.SetServerName()
end

function LoginRolePanel.InitEvent()
    local btnSelect1 = LoginRolePanel.ui["Button_select_1"]
    GUI:Win_SetParam(btnSelect1, 1)

    local btnSelect2 = LoginRolePanel.ui["Button_select_2"]
    GUI:Win_SetParam(btnSelect2, 2)

    local panelRole1 = LoginRolePanel.ui["Panel_role_1"]
    GUI:Win_SetParam(panelRole1, 1)
    
    local panelRole2 = LoginRolePanel.ui["Panel_role_2"]
    GUI:Win_SetParam(panelRole2, 2)

    GUI:addOnClickEvent(btnSelect1, LoginRolePanel.OnSelectRoleEvent)
    GUI:addOnClickEvent(btnSelect2, LoginRolePanel.OnSelectRoleEvent)
    GUI:addOnClickEvent(panelRole1, LoginRolePanel.OnSelectRoleEvent)
    GUI:addOnClickEvent(panelRole2, LoginRolePanel.OnSelectRoleEvent)
end

-- 服务器名
function LoginRolePanel.SetServerName()
    GUI:Text_setString(LoginRolePanel.ui["Text_server_name"], SL:GetMetaValue("SERVER_NAME"))
end

function LoginRolePanel.OnSelectRoleEvent(sender)
    -- 创角/恢复中
    if LoginRolePanel._CreateUI or LoginRolePanel._RestoreUI then
        return false
    end

    local index = GUI:Win_GetParam(sender)
    if not index then
        return false
    end

    local roles = SL:GetLoginData()
...
function LoginRolePanel.UpdateRoles()
    ...
    for i = 1, 2 do
        ...
        if roles[i] then
            ...
            GUI:Text_setString(LoginRolePanel.ui["Text_job_"..i], jobName[roles[i].job + 1])
        end
    end 
end
...
function LoginRolePanel.SelectRole(index, isInit)
    ...
    if not isInit then
        SL:PlayAudio(nil, GUIShare.SND_TYPE_LOGIN_SELECTONE)
    end
    ...
    if animID and animGID and position then
        for i = 1, 2 do
            ...
            local NodeAnim = LoginRolePanel.ui["Node_anim_" .. i]
            GUI:removeAllChildren(NodeAnim)

            if roles[i] then
                if LoginRolePanel._index == i then
                    if isInit then
                        local anim = GUI:Effect_Create(NodeAnim, "role_effect" .. i, position.x, position.y, 0, animID[roles[i].job * 2 + roles[i].sex + 1])
                        GUI:setScale(anim, scale)
                    else
                        local animG = GUI:Effect_Create(NodeAnim, "role_effect_G" .. i, position.x, position.y, 0, animGID[roles[i].job * 2 + roles[i].sex + 1])
                        ...
                        local sfx = GUI:Effect_Create(NodeAnim, "role_sfx" .. i, position.x, position.y, 0, 4114)
...
function LoginRolePanel.ShowDelete()
    local index = LoginRolePanel._index
    ...
    local function callback(bType)
        if bType == 1 then
            ...
            LoginRolePanel.OnDeleteRole(index)
        end
    end
    local data = {}
    data.str = string.format("[%s]删除的角色是不能被恢复的，<br>一段时间内您将不能使用相同的角色名称，<br>你真的确定要删除吗？", roles[index].name)
    data.btnDesc = {"确定", "取消"}
...
function LoginRolePanel.CreateRole(parent)
    ...
    local isRandomJob = SL:IsRandomJob()
    local isRandomSex = SL:IsRandomSex()
    local isSingleJob = SL:IsSingleJob()
    local isSingleSex = SL:GetMetaValue("GAME_DATA", "isSingleSex")


    -- 随机职业
    if not isSingleJob and isRandomJob then
        defaultJob = Random(0, 2)
    end
    -- 随机性别
    ...
    local function updateAnim()
        ...
        local anim = GUI:Effect_Create(ui.Node_anim, "createAnim", pos.x, pos.y, 0, animID[animIdx])
    ...
    if isSingleJob then
        ...
        GUI:setVisible(ui.Button_job_4, false)
    end
...
function LoginRolePanel.createRestore()
    ...
    -- 可恢复角色信息
    local restoreRoles = SL:GetRestoreRoles()
