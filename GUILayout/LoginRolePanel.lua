        ...
        for i = 1, 2 do
            ...
            if roles[i] then
                if LoginRolePanel._index == i then
                    if isInit then
                        local anim = GUI:Effect_Create(LoginRolePanel.ui["Node_anim_" .. i], "role_effect" .. i, position.x, position.y, 0, animID[roles[i].job * 2 + roles[i].sex + 1])
                        GUI:setScale(anim, scale)
                    else
                        local animG = GUI:Effect_Create(LoginRolePanel.ui["Node_anim_" .. i], "role_effect_G" .. i, position.x, position.y, 0, animGID[roles[i].job * 2 + roles[i].sex + 1])
...
function LoginRolePanel.createRole(parent)
    ...
    -- 随机职业
    if not SL:GetMetaValue("GAME_DATA", "isSingleJob") and SL:GetMetaValue("GAME_DATA", "isRandomJob") then
        defaultJob = Random(0, 2)
    end
    -- 随机性别
    if not SL:GetMetaValue("GAME_DATA", "isSingleSex") and SL:GetMetaValue("GAME_DATA", "isRandomSex") then
    ...
    if SL:GetMetaValue("GAME_DATA", "isSingleSex") then
        if SL:GetMetaValue("GAME_DATA","isSingleSex") == "boy" then
            isSingleSex = 0
        elseif SL:GetMetaValue("GAME_DATA","isSingleSex") == "girl" then
            isSingleSex = 1
        end
    end
