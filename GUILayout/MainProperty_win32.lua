...
function MainProperty.InitAdapet()
    ...
    GUI:setPositionX(MainProperty._ui.Panel_chat, screenW / 2)
    GUI:setContentSize(
...
function MainProperty.InitActPanel()
    ...
    GUI:addMouseOverTips(
        ...
        string.format("当前等级：%s", SL:GetMetaValue("LEVEL")),
        {x = 0, y = -20},
        {x = 0.1, y = 0.5}
    )
    GUI:addMouseOverTips(
...
function MainProperty.InitChatPanel()
    ...
    if not tradeAble then
        ...
        GUI:setPositionX(MainProperty._ui.btn_rein_add, 149)
    end
...
function MainProperty.RefAnger(value)
    ...
    GUI:setContentSize(MainProperty._ui.Panel_loadBar, size.width, 5 + value * 150)
end
