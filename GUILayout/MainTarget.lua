...
function MainTarget.main(...)
    ...
    GUI:setVisible(MainTarget._panel, false)
...
function MainTarget.SelectTarget(targetID)
    ...
    GUI:setVisible(MainTarget._panel, true)
    -- icon
...
function MainTarget.UpdateTargetHP()
    ...
    local state = SL:GetMetaValue("ACTOR_ATTACK_STATE", target)
    if SL:GetMetaValue("GAME_DATA", "showFewHp") == 0 and type(state) == "boolean" and not state and MainTarget._targetID ~= SL:GetMetaValue("HERO_ID") then
        percent = 100
    end
...
function MainTarget.OnUpdateLockBtn()
    ...
    if SL:GetMetaValue("PET_ALIVE") then
