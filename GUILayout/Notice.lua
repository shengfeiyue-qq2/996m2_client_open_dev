...
function Notice.ShowServerEventNotice_11(data)
    ...
    local function showServerEvent()
        ...
        local layout = GUI:Layout_Create(Notice._rootServerTips, "layout11"..Notice._serverIndex11, visibleSize.width/2, -30, capacitySize.width, capacitySize.height, true)
...
function Notice.OnShowTimerNotice(data)
    ...
    local layout = GUI:Layout_Create(Notice._listviewTimerTips, "layoutTimer"..Notice._timerIndex, 0, 0, capacitySize.width, capacitySize.height, true)
    resetListview()
...
function Notice.OnShowTimerXYNotice(data)
    ...
    local layout = GUI:Layout_Create(Notice._rootTimerTipsXY, "layoutXYTimer"..Notice._timerXYIndex, 0, 0, capacitySize.width, capacitySize.height, true)
    if data.X == 0 then
...
function Notice.OnShowPlayerEXPNotice(data)
    ...
    local visibleSize = SL:GetMetaValue("SCREEN_SIZE")
    Y = visibleSize.height - Y
    GUI:setPosition(Notice._expNode, X, Y)
