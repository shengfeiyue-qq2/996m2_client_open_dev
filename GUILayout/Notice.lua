...
function Notice.fixOutline(str, color)
    return string.format("<outline size='1' color='%s'>%s</outline>", color, str)
end
...
function Notice.CheckServerNotice()
    ...
    local FColorRGB = SL:GetColorByID(data.FColor)
    local BColorRGB = SL:GetColorByID(data.BColor)
    
    -- richText
    Notice._serverIndex = Notice._serverIndex + 1

    local richText = GUI:RichText_Create(Notice._rootServerTips, "richTextServer"..Notice._serverIndex, 0, 0, Notice.fixOutline(data.Msg, BColorRGB), 10000, 18, FColorRGB)
...
function Notice.ShowServerEventNotice_11(data)
    ...
    local function showServerEvent()
        ...
        local FColorRGB     = SL:GetColorByID(item.FColor)
        local BColorRGB     = SL:GetColorByID(item.BColor)
        local visibleSize   = SL:GetMetaValue("SCREEN_SIZE")
        ...
        local richText = GUI:RichText_Create(layout, "richText", 0, 0, Notice.fixOutline(item.Msg, BColorRGB), 10000, fontSize, FColorRGB)
...
function Notice.OnShowSystemNotice(data)
    ...
    local function showSystemNotice()
        ...
        -- richText
        local fontSize = SL:GetMetaValue("GAME_DATA", "DEFAULT_FONT_SIZE_NOTICE") or 16
        local richText = GUI:RichText_Create(layout, "richText", capacitySize.width, capacitySize.height / 2, Notice.fixOutline(item.Msg, BColorRGB), 10000, fontSize, FColorRGB)
        GUI:setAnchorPoint(richText, 0, 0.5)
...
function Notice.OnShowSystemScaleNotice(data)
    ...
    local function showSystemScaleNotice()
        ...
        local fontSize = SL:GetMetaValue("GAME_DATA", "DEFAULT_FONT_SIZE_NOTICE") or 16
        local richText = GUI:RichText_Create(imageBG, "richText", contentSize.width, contentSize.height / 2, Notice.fixOutline(item.Msg, BColorRGB), 10000, fontSize, FColorRGB)
...
function Notice.OnShowSystemXYNotice(data)
    ...
    local FColorRGB = SL:GetColorByID(data.FColor or 255)
    local BColorRGB = SL:GetColorByID(data.BColor or 255)
    ...
    -- richText
    local fontSize = SL:GetMetaValue("GAME_DATA", "DEFAULT_FONT_SIZE_NOTICE") or 16
    local richText = GUI:RichText_Create(node, "richText", 0, 0, Notice.fixOutline(data.Msg, BColorRGB), 10000, fontSize, FColorRGB)
    GUI:setCascadeOpacityEnabled(richText, true)
    GUI:setAnchorPoint(richText, 0, 1)
...
function Notice.OnShowTimerNotice(data)
    ...
    local function callback()
        ...
        -- richText
        local fontSize = SL:GetMetaValue("GAME_DATA", "DEFAULT_FONT_SIZE_NOTICE") or 16
        local richText = GUI:RichText_Create(layout, "richText", 0, 0, Notice.fixOutline(str, BColorRGB), 10000, fontSize, FColorRGB)
        GUI:setAnchorPoint(richText, 0.5, 0.5)
...
function Notice.OnShowTimerXYNotice(data)
    ...
    local function callback()
        ...
        local richText = GUI:RichText_Create(layout, "richText", capacitySize.width / 2, capacitySize.height / 2, Notice.fixOutline(str, BColorRGB), 10000, fontSize, FColorRGB)
        GUI:setAnchorPoint(richText, 0.5, 0.5)
...
function Notice.OnShowItemTips(info)
    ...
    local function showTips()
        ...
        -- richText
        if Notice.isPC then 
            local text = GUI:BmpText_Create(node, "text", 0, 0, data.color, str)
            if data.heroStr then 
                local text2 = GUI:BmpText_Create(node, "text2", -4, 0, data.heroColor, data.heroStr)
...
function Notice.CheckNoticeAttribute()
    ...
    for k, v in pairs(attributes) do
        if v and v.id then
            pdata[tonumber(v.id)] = v.value
        end
    end
    local attr_data = {}
    local showAttrs = GUIShare.ParseAttList(pdata)
    GUIShare.AttSort(showAttrs)
    ...
    for i, v in ipairs(showAttrs) do
        ...
        attr_data[#attr_data + 1] = {name = v.name, attr = v.value}
    end
