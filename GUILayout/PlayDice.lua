...
function PlayDice.main()
    local data = GUI:GetLayerOpenParam()
    local parent = GUI:Win_Create(UIConst.LAYERID.PlayDiceGUI, 0, 0, 0, 0, false, false, true, true)
    ...
    PlayDice.RegisterEvent()
end

function PlayDice.OnClose(UID)
    if UID ~= UIConst.LAYERID.PlayDiceGUI then
        return false
    end

    SL:SubmitAct({npcid = PlayDice._data.npcid, Act = PlayDice._data.callback})

    PlayDice.UnRegisterEvent()
end

function PlayDice.RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_CLOSEWIN, "PlayDice", PlayDice.OnClose)                                 -- 关闭界面
end

function PlayDice.UnRegisterEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_CLOSEWIN, "PlayDice")
end
