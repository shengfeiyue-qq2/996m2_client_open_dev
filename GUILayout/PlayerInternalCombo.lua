...
function PlayerInternalCombo.RefreshSetIconShow(key)
    ...
    if selectSkills[key] and selectSkills[key] ~= 0 then
        local iconPath  = SL:GetMetaValue("SKILL_RECT_ICON_PATH", selectSkills[key]) 
        if SL:IsFileExist(iconPath) then
            GUI:Image_loadTexture(PlayerInternalCombo._ui["skill_icon_" .. key], iconPath)
        end
    else
...
function PlayerInternalCombo.UpdateSkillCells()
    ...
    for i, v in ipairs(items) do
        ...
        local cell = PlayerInternalCombo.CreateSkillCell(PlayerInternalCombo._ui.ListView_cells, skillID)
        PlayerInternalCombo._cells[skillID] = cell
    end
...
function PlayerInternalCombo.CreateSkillCell(parent, skillID)
    PlayerInternalCombo._index = PlayerInternalCombo._index + 1
    local cellW = PlayerInternalCombo._isWIN32 and 244 or 310
    local cellH = PlayerInternalCombo._isWIN32 and 50 or 70
    local widget = GUI:Widget_Create(parent, "SkillItems_" .. PlayerInternalCombo._index, 0, 0, cellW, cellH)
    if PlayerInternalCombo._isWIN32 then
        GUI:LoadExport(widget, "internal_player/combo_skill_cell_win32")
    else
        GUI:LoadExport(widget, "internal_player/combo_skill_cell")
    end
