...
function PlayerInternalSkill.UpdateSkillCells()
    ...
    for i, v in ipairs(items) do
        ...
        local cell = PlayerInternalSkill.CreateSkillCell(PlayerInternalSkill._ui.ListView_cells, skillID, skillType)
        PlayerInternalSkill._cells[string.format("%s_%s", skillID, skillType)] = cell
    end

    PlayerInternalSkill.RefreshSkillCells()
end
...
function PlayerInternalSkill.CreateSkillCell(parent, skillID, skillType)
    PlayerInternalSkill._index = PlayerInternalSkill._index + 1
    local cellW = PlayerInternalSkill._isWIN32 and 244 or 310
    local cellH = PlayerInternalSkill._isWIN32 and 50 or 70
    local widget = GUI:Widget_Create(parent, "SkillItems_" .. PlayerInternalSkill._index, 0, 0, cellW, cellH)
    if PlayerInternalSkill._isWIN32 then
        GUI:LoadExport(widget, "internal_player/skill_cell_win32")
    else
        GUI:LoadExport(widget, "internal_player/skill_cell")
    end
    ...
    GUI:addOnClickEvent(ui.Button_onoff, function(sender)
        ...
        local path = PlayerInternalSkill._path .. (state == 0 and "btn_jnkan_02.png" or "btn_jnkan_01.png")
        local img = sender:getChildByName("Image_onoff")
