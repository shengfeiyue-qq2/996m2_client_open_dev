...
function SettingAddMonsterName.InitUI(data)
    ...
    GUI:setPosition(ui.Panel_1, Panel_Size.width / 2 + 50, ScreenHeight / 2)
    ...
    local monsters = {}
    local playerVec = SL:FindPlayerInCurrViewField(true)--获取视野内的玩家
    for _, player in ipairs(playerVec) do
        if player:IsHumanoid() then--人形怪
            if true == SL:checkLaunchTargetByID(player:GetID()) then --可攻击
                monsters[player:GetName()] = 1
            end
        end
    end
    local monsterVec = SL:FindMonsterInCurrViewField(true, true)--获取视野内的怪物
    for _, monster in ipairs(monsterVec) do
        if true == SL:checkLaunchTargetByID(monster:GetID()) then
            monsters[monster:GetName()] = 1
        end
    end
