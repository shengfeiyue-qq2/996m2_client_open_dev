...
function UIModel.main(sex, feature, scale, params)
    ...
    if feature.embattlesID and next(feature.embattlesID) then
        ...
        for k,id in pairs(feature.embattlesID) do
            local embattleEffectAnim = GUI:Effect_Create(baseNode, "embattles", sfxX, sfxY, 0, id, 0, 0, 0, 1)
    ...
    if showCloth then
        ...
        if showClothEffect and showClothEffect ~= "0" and showClothEffect ~= "" then
            ...
            for i, v in ipairs(effectList) do
                -- 衣服特效
                local clothEffectAnim = GUI:Effect_Create(node, "clothAnim", v.offX, - v.offY, 0, v.effectId)
    ...
    if showWeapon then
        ...
        if showWeaponEffect and showWeaponEffect ~= "0" and showWeaponEffect ~= "" then
            ...
            for i, v in ipairs(effectList) do
                -- 武器特效
                local weaponEffectAnim = GUI:Effect_Create(node, "weaponAnim", v.offX, - v.offY, 0, v.effectId)
    ...
    if veilId then
        ...
        if veilEffect and veilEffect ~= "0" and veilEffect ~= "" then
            ...
            for i, v in ipairs(effectList) do
                local veilEffectAnim = GUI:Effect_Create(node, "veilAnim", v.offX, - v.offY, 0, v.effectId)
    ...
    if shieldID then
        ...
        if shieldEffect and shieldEffect ~= "0" and shieldEffect ~= "" then
            ...
            for i,v in ipairs(effectList) do
                -- 盾牌特效
                local shieldEffectAnim = GUI:Effect_Create(node, "shieldAnim", v.offX, - v.offY, 0, v.effectId)
