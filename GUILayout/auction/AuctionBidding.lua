...
function AuctionBidding.main()
    ...
    GUI:LoadExport(parent,
        SL:GetValue("IS_PC_OPER_MODE") and "auction_win32/auction_bidding" or "auction/auction_bidding")
...
function AuctionBidding.CreateItemCell(parent, data)
    GUI:LoadExport(parent,
        SL:GetValue("IS_PC_OPER_MODE") and "auction_win32/auction_bidding_cell" or "auction/auction_bidding_cell")
    ...
    data = AuctionBidding._itemList[data.item.MakeIndex]
    ...
    local function callback()
        local status, remaining = SL:GetValue("AUCTION_ITEM_STATE", data)
...
function AuctionBidding.UpdateBiddingList(items)
    ...
    if items and next(items) then
        while next(items) do
            ...
            local quickCell = GUI:QuickCell_Create(AuctionBidding._ui.ListView_items, "item_" .. item.item.MakeIndex, 0, 0,
                AuctionBidding._itemSize.width, AuctionBidding._itemSize.height, createCell)
            AuctionBidding._quickCells[item.item.MakeIndex] = quickCell
...
function AuctionBidding.OnAuctionItemAdd(data)
    ...
    local mainPlayerID = SL:GetValue("USER_ID")
    if item.joinuser == 1 and (item.btFlag == 0 or (item.btFlag == 1 and item.sCurUser == mainPlayerID)) then
...
function AuctionBidding.OnAuctionItemDel(data)
    local item = data
    -- not exist
    if not AuctionBidding._itemList[item.item.MakeIndex] or not AuctionBidding._quickCells[item.item.MakeIndex] then
        return nil
    end
    local cell = AuctionBidding._quickCells[item.item.MakeIndex]
    ...
    AuctionBidding._quickCells[item.item.MakeIndex] = nil
    AuctionBidding._itemList[item.item.MakeIndex] = nil
...
function AuctionBidding.OnAuctionBiddingListResp(items)
    AuctionBidding.UpdateBiddingList(items)
end

function AuctionBidding.OnAuctionWorldItemDel(data)
    if data and data.item then
        AuctionBidding.OnAuctionItemDel(data.item)
    end
end

function AuctionBidding.OnAuctionWorldItemChange(data)
    if data and data.item then
...
-- 界面关闭回调
function AuctionBidding.OnClose()
    AuctionBidding.UnRegisterEvent()
...
function AuctionBidding.RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_WORLD_ITEM_ADD, "AuctionBidding", AuctionBidding.OnAuctionItemAdd) -- 刷新物品
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_BIDDING_LIST, "AuctionBidding", AuctionBidding.OnAuctionBiddingListResp)
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_WORLD_ITEM_DEL, "AuctionBidding", AuctionBidding.OnAuctionWorldItemDel)
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_WORLD_ITEM_CHANGE, "AuctionBidding", AuctionBidding.OnAuctionWorldItemChange)
end

function AuctionBidding.UnRegisterEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_WORLD_ITEM_ADD, "AuctionBidding")
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_BIDDING_LIST, "AuctionBidding")
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_WORLD_ITEM_DEL, "AuctionBidding")
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_WORLD_ITEM_CHANGE, "AuctionBidding")
end
