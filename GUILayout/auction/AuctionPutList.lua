...
function AuctionPutList.main()
    ...
    GUI:LoadExport(parent,
        SL:GetValue("IS_PC_OPER_MODE") and "auction_win32/auction_put_list" or "auction/auction_put_list")

    AuctionPutList._ui = GUI:ui_delegate(parent)

    AuctionPutList._showBagItems = SL:GetValue("AUCTION_MY_SHOW_LIST")
    ...
    AuctionPutList:RegisterEvent()

    SL:AttachTXTSUI({
        root  = AuctionPutList._ui.Panel_1,
        index = SLDefine.SUIComponentTable.AuctionPutlist
    })
end
...
function AuctionPutList.InitPutList()
    ...
    local limitCount = SL:GetValue("AUCTION_DEFAULT_SHELF")
    ...
    for i = 1, limitCount do
        ...
        local cell = GUI:QuickCell_Create(AuctionPutList._ui.ScrollView_items, "item_" .. i, 0, 0, wid, hei, createCell,
...
function AuctionPutList.CreatePutCell(parent, index)
    ...
    if not item then
        ...
        if bidAble then
            local itemShow = GUI:ItemShow_Create(ui.Node_money, "money_item", 0, 0, item.btType)
            GUI:setAnchorPoint(itemShow, 0.5, 0.5)
            GUI:setScale(itemShow, 0.5)

            local fixBidPrice = GUIFunction:FixAuctionPrice(item.nCurPrice, true)
        ...
        if buyAble then
            local itemShow = GUI:ItemShow_Create(ui.Node_buy_money, "money_item", 0, 0, item.btType)
            GUI:setAnchorPoint(itemShow, 0.5, 0.5)
            GUI:setScale(itemShow, 0.5)

            local fixBuyPrice = GUIFunction:FixAuctionPrice(item.nLastPrice, true)
...
function AuctionPutList.UpdateShelfCount()
    -- local count = SL:GetValue("AUCTION_PUT_LIST_CNT")
    local count = #AuctionPutList._items
    GUI:Text_setString(AuctionPutList._ui.Text_tile,
        string.format("寄售货架(%s/%s)", count, SL:GetValue("AUCTION_DEFAULT_SHELF")))
end
...
function AuctionPutList.OnPutInItem(item)
    ...
    for k, v in ipairs(AuctionPutList._items) do
        if v.item.MakeIndex == item.item.MakeIndex then
...
function AuctionPutList.OnPutOutItem(item)
    ...
    for k, v in ipairs(AuctionPutList._items) do
        if v.item.MakeIndex == item.makeindex then
...
function AuctionPutList.InitBagList()
    ...
    for i = 1, totalNum do
        ...
        local cell = GUI:QuickCell_Create(AuctionPutList._ui.ScrollView_bag, "item_" .. i, 0, 0, wid, hei, createCell,
            checkActive)
...
function AuctionPutList.CreateItemCell(parent)
    GUI:LoadExport(parent,
        SL:GetValue("IS_PC_OPER_MODE") and "auction_win32/auction_put_list_cell" or "auction/auction_put_list_cell")
end

-- 空列表 cell
function AuctionPutList.CreateEmptyCell(parent)
    GUI:LoadExport(parent,
        SL:GetValue("IS_PC_OPER_MODE") and "auction_win32/auction_put_list_empty_cell" or
        "auction/auction_put_list_empty_cell")
end

-- 背包物品 cell
function AuctionPutList.CreateBagCell(parent, i)
    GUI:LoadExport(parent,
        SL:GetValue("IS_PC_OPER_MODE") and "auction_win32/auction_put_list_bag_cell" or
        "auction/auction_put_list_bag_cell")
    ...
    if itemData then
        ...
        GUI:ItemShow_addReplaceClickEvent(item, function()
            ...
            UIOperator:OpenAuctionPutInUI(data)
        end)
    end
    return cell
end

function AuctionPutList.OnAuctionPutinResp(data)
    local item = data.item
    AuctionPutList.OnPutInItem(item)
end

function AuctionPutList.OnAuctionPutListResp(items)
    AuctionPutList.OnUpdatePutList(items)
end

function AuctionPutList.OnAuctionPutoutResp(item)
    AuctionPutList.OnPutOutItem(item)
end

-- 界面关闭回调
function AuctionPutList.OnClose()
    AuctionPutList.UnRegisterEvent()
...
function AuctionPutList.RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_CLOSEWIN, "AuctionPutList", AuctionMain.OnClose)
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_PUT_IN, "AuctionPutList", AuctionPutList.OnAuctionPutinResp)     --上架道具
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_PUT_LIST, "AuctionPutList", AuctionPutList.OnAuctionPutListResp)
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_PUT_OUT, "AuctionPutList", AuctionPutList.OnAuctionPutoutResp)
end

function AuctionPutList.UnRegisterEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_CLOSEWIN, "AuctionPutList")
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_PUT_IN, "AuctionPutList")   --上架道具
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_PUT_LIST, "AuctionPutList")
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_PUT_OUT, "AuctionPutList")
end
