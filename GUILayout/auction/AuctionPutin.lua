...
function AuctionPutin.main()
    local parent = GUI:Win_Create(UIConst.LAYERID.AuctionPutinGUI, 0, 0, 0, 0, false, false, true, true)
    ...
    AuctionPutin._currcies       = SL:GetValue("AUCTION_MONEY")
    ...
    GUI:addOnClickEvent(AuctionPutin._ui["Button_submit"], function(sender)
        ...
        -- 货架满
        if SL:GetValue("AUCTION_PUT_LIST_CNT") >= SL:GetValue("AUCTION_DEFAULT_SHELF") then
        ...
        SL:RequestAuctionPutInItem(itemData.MakeIndex, inputCount, inputBidPrice, inputBuyPrice, currencyID, rebate.value)        
    end)
    ...
    -- 注册事件监听
    AuctionPutin:RegisterEvent()

    SL:AttachTXTSUI({
        root  = AuctionPutin._ui.Panel_2,
        index = SLDefine.SUIComponentTable.AuctionPutin
    })
end
...
function AuctionPutin.CreateCurrencyCell()
    ...
    GUI:LoadExport(parent, SL:GetValue("IS_PC_OPER_MODE") and "auction_win32/auction_putin_currency_cell" or "auction/auction_putin_currency_cell")
...
function AuctionPutin.Close()
    GUI:Win_CloseByID(UIConst.LAYERID.AuctionPutinGUI)
end

-- 界面关闭回调
function AuctionPutin.OnClose()
    AuctionPutin.UnRegisterEvent()
...
function AuctionPutin.RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_AUCTION_PUT_IN, "AuctionPutin", AuctionPutin.Close)                  -- 更新红点
end

function AuctionPutin.UnRegisterEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_AUCTION_PUT_IN, "AuctionPutin")
end
