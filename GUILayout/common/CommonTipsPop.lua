...
function CommonTipsPop.main()
    ...
    local parent = GUI:Win_Create(UIConst.LAYERID.CommonTipsGUI, 0, 0, 0, 0, false, false, true, true, nil, nil, GUIDefine.UIZ.TOBOX, CommonTipsPop.HandlePressEnter)
...
function CommonTipsPop.InitDesc()
    ...
    local commonTipsFontPath = SL:GetValue("IS_PC_OPER_MODE") and SL:GetValue("GAME_DATA", "COMMONTIPS_FONT_PATH")
    local richText = GUI:RichText_Create(CommonTipsPop._ui.DescList, "desc_text", 0, 0, CommonTipsPop._data.str, limitWid, SL:GetValue("GAME_DATA", "DEFAULT_FONT_SIZE"), "#FFFFFF", nil, nil, commonTipsFontPath)
    if richText then
...
function CommonTipsPop.HandlePressEnter()
    if SL:GetValue("IS_PC_OPER_MODE") and not CommonTipsPop._showEdit then
