    ...
    local function onRefEquipNaikan()
        ...
        if state then
            ...
            if pos == EquipPosCfg.Equip_Type_Super_Dress and tonumber(SL:GetValue("GAME_DATA", "Fashionfx") or 0) ~= 1 then
                HeroSuperEquip._feature.showNodeModel = true
                HeroSuperEquip._feature.showHair = true
            end
        else
            HeroSuperEquip.UpdateModelFeatureData()
        end
    ...
    for _, pos in ipairs(HeroSuperEquip._EquipPosSet) do
        ...
        if widget and GUI:getVisible(widget) then
            ...
            local function onRightDownFunc(touchPos)
                ...
                -- 开始
                HeroSuperEquip.UpdateMoveState(widget, true, pos)
                SL:onLUAEvent(LUA_EVENT_LAYER_MOVED_BEGIN, {
                    ...
                    cancelCallBack = function ()
                        ...
                        HeroSuperEquip.UpdateMoveState(widget, false, pos)
                    end
                })
            end
...
function HeroSuperEquip.InitEquipCells()
    ...
    if openFEquip and openFEquip == 0 then
        ...
        HeroSuperEquip._EquipPosSet = {17, 18}
    else
        -- 额外的装备位置 1是6格 0是4格
