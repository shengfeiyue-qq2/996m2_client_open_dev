...
function HeroInternalSkill.main()
    ...
    GUI:LoadExport(parent, isPC and "internal_hero/internal_skill_node_win32" or "internal_hero/internal_skill_node")
    ...
    SL:AttachTXTSUI({root = HeroInternalSkill._ui["Panel_1"], index = SLDefine.SUIComponentTable.HeroInternalSkill})
end
...
function HeroInternalSkill.UpdateSkillCells()
    ...
    --已学的技能
    local items = SL:CopyData(SL:GetValue("H.INTERNAL_SKILLS"))
...
function HeroInternalSkill.UpdateSkillCell(key)
    ...
    local skill =  SL:GetValue("H.INTERNAL_SKILL_DATA", skillID, skillType)
    if not skill then
        return
    end
    -- icon
    local contentSize = GUI:getContentSize(cell.Image_icon)
    local iconPath  = SL:GetValue("H.INTERNAL_SKILL_RECT_ICON_PATH", skillID, skillType) 
    ...
    -- 熟练度 等级
    local strTrain = SL:GetValue("H.INTERNAL_SKILL_TRAIN_DATA", skillID, skillType) 
    ...
    -- 开关显示
    local state = SL:GetValue("H.INTERNAL_SKILL_ONOFF", skillID, skillType)
    GUI:Button_setBright(cell.Button_onoff, state == 0)
    local path = resPath .. (state == 0 and "btn_jnkan_02.png" or "btn_jnkan_01.png")
...
function HeroInternalSkill.CreateSkillCell(parent, skillID, skillType)
    GUI:LoadExport(parent, isPC and "internal_hero/skill_cell_win32" or "internal_hero/skill_cell")
    local ui = GUI:ui_delegate(parent)
    local cell = GUI:getChildByName(parent, "Panel_skill_cell")

    local name = SL:GetValue("H.INTERNAL_SKILL_NAME", skillID, skillType) 
    ...
    GUI:addOnClickEvent(ui.Image_icon, function(sneder)
        local desc = SL:GetValue("H.INTERNAL_SKILL_DESC", skillID, skillType)
        if desc then
    ...
    GUI:addOnClickEvent(ui.Button_onoff, function(sender)
        ...
        SL:SetValue("H.INTERNAL_SKILL_ONOFF", skillID, skillType, state)
    end)
    return cell, ui
end
...
function HeroInternalSkill.OnClose()
    ...
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.HeroInternalSkill
    })
end
