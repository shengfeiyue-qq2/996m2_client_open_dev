LookHeroTitle = {}
LookHeroTitle._ui = nil

local isPC = SL:GetValue("IS_PC_OPER_MODE")

LookHeroTitle._path = GUIDefine.PATH_RES_PRIVATE .. (isPC and "title_layer_ui_win32/" or "title_layer_ui/")

function LookHeroTitle.main()
    local parent = GUI:GetLayerOpenParam()
    GUI:SetLayerOpenParam(nil)
    GUI:LoadExport(parent, isPC and "hero_look/hero_title_node_win32" or "hero_look/hero_title_node")

    LookHeroTitle._parent = parent
    LookHeroTitle._ui = GUI:ui_delegate(parent)
    if not LookHeroTitle._ui then
        return false
    end

    LookHeroTitle.InitUI()

    SL:AttachTXTSUI({root = parent, index = SLDefine.SUIComponentTable.TitleO_hero})
end

-- 界面关闭回调
function LookHeroTitle.OnClose()
    -- 自定义组件卸载
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.TitleO_hero
    })
end

function LookHeroTitle.InitUI()
    -- 已激活的称号
    local activeID = LookPlayerData.GetActiveTitle()

    LookHeroTitle.RefCurTitle(activeID)

    LookHeroTitle.RefTitleList(activeID)
end

-- 刷新当前穿戴
function LookHeroTitle.RefCurTitle(activeID)
    local btnCurTitle = LookHeroTitle._ui["Button_curTitle"]
    local lblCurTitle = LookHeroTitle._ui["Text_curTitle"]
...
-- 刷新称号列表
function LookHeroTitle.RefTitleList(activeID)
    -- 称号数据
    ...
    local list = LookHeroTitle._ui["ListView_cells"]
    ...
    for i = 1, count do
        ...
        if titleList[i] then
            ...
            local res = LookHeroTitle._path .. "title_4.png"
            GUI:Button_loadTextureNormal(buttonIcon, res)
