...
function HeroSuperEquip_Look_TradingBank.InitEquipLayerEvent()
    for _,pos in pairs(HeroSuperEquip_Look_TradingBank._EquipPosSet) do
        local widget = HeroSuperEquip_Look_TradingBank.GetEquipPosPanel(pos)
        if widget then
            GUI:setTouchEnabled(widget, true)
            GUI:addOnTouchEvent(widget, function (sender, eventType) HeroSuperEquip_Look_TradingBank.OnClickEvent() end)
            ...
            local Node = HeroSuperEquip_Look_TradingBank.GetEquipPosNode(pos)
            if Node then
...
function HeroSuperEquip_Look_TradingBank.GetLooks(pos)
    ...
    -- 通过唯一ID MakeIndex 获取装备数据
    if pos == EquipPosCfg.Equip_Type_Super_Dress and equipData and HeroSuperEquip_Look_TradingBank._feature.showNodeModel and equipData.shonourSell and tonumber(equipData.shonourSell) == 1 then -- shonourSell == 1 不显示裸模, 服务器下发字段
...
function HeroSuperEquip_Look_TradingBank.OnOpenItemTips(widget, pos)
    ...
    local itemData = HeroSuperEquip_Look_TradingBank.IsNaikan(pos) and GUIFunction:GetEquipDataListByPos(pos, EDType) or {GUIFunction:GetEquipDataByPos(pos, nil, EDType)}
