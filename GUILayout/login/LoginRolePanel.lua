...
function LoginRolePanel.main()
    ...
    for k,v in pairs(equipConfig) do
        if not LoginRolePanel._equipConfig[v.Index] then
            LoginRolePanel._equipConfig[v.Index] = v
        end
    end
...
function LoginRolePanel.NewCreateRole(looks, node , index)
    if looks then
        ...
        local weaponData = weaponIndex and LoginRolePanel._equipConfig[tonumber(weaponIndex)] or nil
        local helmetData = helmetIndex and LoginRolePanel._equipConfig[tonumber(helmetIndex)] or nil
        local dressData = dressIndex and LoginRolePanel._equipConfig[tonumber(dressIndex)] or nil
        local capData = capsIndex and LoginRolePanel._equipConfig[tonumber(capsIndex)] or nil
        local veilData = veilIndex and LoginRolePanel._equipConfig[tonumber(veilIndex)] or nil
        local shieldData = shieldIndex and LoginRolePanel._equipConfig[tonumber(shieldIndex)] or nil
        ...
        local tempWidget = node
        local scale = LoginRolePanel._animScale[index] 

        local UIModel = GUI:UIModel_Create(node, "role_effect"..index, 0, 150, sex, modelData, scale, true, tonumber(looks.Job),{ext_node = tempWidget})
        return UIModel
    end
end
...
function LoginRolePanel.SetModelGrey(model, isGrey)
    ...
    local function setNodeGrey(node)
        ...
        local children = GUI:getChildren(node)
        if children then
...
function LoginRolePanel.OnCreateRoleFail(errorCode)
    ...
    elseif errorCode == 999 then
        SL:ShowSystemTips("您当前暂时无法创建新的角色！")
    else
