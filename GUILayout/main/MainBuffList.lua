...
function MainBuffList.main()
    ...
    local showBuffList = SL:GetValue("GAME_DATA", "ShowBuffList")
    if showBuffList and string.len(showBuffList) > 0 then
        local set = string.split(showBuffList, "|")
        local params = string.split(SL:GetValue("IS_PC_OPER_MODE") and set[2] or set[1], "#")
...
function MainBuffList.CreateBuffPanel(data)
    ...
    -- 往下收缩 4
    local dirParam = {
        [1] = {anr = GUI:p(0, 0.5), pos = GUI:p(panelWid, panelHei * 0.5)},
        [2] = {anr = GUI:p(1, 0.5), pos = GUI:p(- panelWid, panelHei * 0.5)},
        [3] = {anr = GUI:p(0.5, 1), pos = GUI:p(panelWid * 0.5, - panelHei)},
        [4] = {anr = GUI:p(0.5, 0), pos = GUI:p(panelWid * 0.5, panelHei)},
    }
    ...
    if MainBuffList._foldBtnDir and MainBuffList._foldBtnDir ~= 0 then
        ...
        GUI:addOnClickEvent(foldBtn, function()
            ...
            if MainBuffList._foldBtnDir == 1 or MainBuffList._foldBtnDir == 2 then
                local movePos = GUI:p( - (MainBuffList._hide and pos.x or 0), 0)
                ...
                local btnMovePos = GUI:p(pos.x - (MainBuffList._hide and pos.x or 0), pos.y)
                ...
                local movePos = GUI:p(0, - (MainBuffList._hide and pos.y or 0))
                GUI:Timeline_EaseSineIn_MoveTo(panel, movePos, 0.2, function()
                    GUI:setVisible(MainBuffList._buffList, not MainBuffList._hide)
                end)
                local btnMovePos = GUI:p(pos.x, pos.y - (MainBuffList._hide and pos.y or 0))
                GUI:Timeline_MoveTo(foldBtn, btnMovePos, 0.2, function()
                    GUI:Button_loadTextureNormal(foldBtn, string.format(foldPathF, MainBuffList._hide and "_1" or ""))
                end)
            end
    ...
    SL:RegisterLUAEvent(LUA_EVENT_MAINPLAYER_BUFF_UPDATE, "MainBuffList", MainBuffList.OnUpdateBuff)
end
...
function MainBuffList.OnFillContent(isAutoCol)
    local items = SL:GetValue("ACTOR_BUFF_DATA")
...
function MainBuffList.OnUpdateBuff(data)
    if SL:GetValue("USER_ID") ~= data.actorID then
        return
    end
