...
function PlayerInternalCombo.main()
    ...
    GUI:LoadExport(parent, isPC and "internal_player/internal_combo_node_win32" or "internal_player/internal_combo_node")
    ...
    SL:AttachTXTSUI({root = PlayerInternalCombo._ui["Panel_1"], index = SLDefine.SUIComponentTable.PlayerInternalCombo})
end

function PlayerInternalCombo.InitSelectSkill()
    local selectSkills = SL:GetValue("SET_COMBO_SKILLS")
    local openNum = SL:GetValue("OPEN_COMBO_NUM")
    for i = 1, 4 do
...
function PlayerInternalCombo.ShowSelectPanel(key, pos)
    local list = {}
    local items = SL:CopyData(SL:GetValue("HAVE_COMBO_SKILLS"))
    items = SL:HashToSortArray(items, function(a, b)
        return a.MagicID < b.MagicID
    end)
    for _, item in ipairs(items) do
        local name = SL:GetValue("SKILL_NAME", item.MagicID)
...
function PlayerInternalCombo.UpdateSkillCell(skillID)
    ...
    local skill =  SL:GetValue("COMBO_SKILL_DATA", skillID) 
    if not skill then
        return
    end
    -- name 暴击率
    local nameStr = SL:GetValue("SKILL_NAME", skillID)
    ...
    -- 熟练度 等级
    local strTrain = SL:GetValue("COMBO_SKILL_TRAIN_DATA", skillID)
...
function PlayerInternalCombo.OnClose()
    ...
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalCombo
    })
end
