...
function PlayerInternalMeridian.main()
    ...
    GUI:LoadExport(parent, isPC and "internal_player/internal_meridian_node_win32" or "internal_player/internal_meridian_node")
    ...
    refreshBtnShow()
    -- 固定文本尺寸
    GUI:Text_setTextAreaSize(PlayerInternalMeridian._ui.Text_show, isPC and {width = 12, height = 60} or {width = 16, height = 76})
    ...
    -- 自定义组件挂接
    SL:AttachTXTSUI({root = PlayerInternalMeridian._ui["Panel_1"], index = SLDefine.SUIComponentTable.PlayerInternalMeridian})
    SL:AttachTXTSUI({root = PlayerInternalMeridian._ui["Panel_show_"..1], index = SLDefine.SUIComponentTable.PlayerInternalMeridian1})
    SL:AttachTXTSUI({root = PlayerInternalMeridian._ui["Panel_show_"..2], index = SLDefine.SUIComponentTable.PlayerInternalMeridian2})
    SL:AttachTXTSUI({root = PlayerInternalMeridian._ui["Panel_show_"..3], index = SLDefine.SUIComponentTable.PlayerInternalMeridian3})
    SL:AttachTXTSUI({root = PlayerInternalMeridian._ui["Panel_show_"..4], index = SLDefine.SUIComponentTable.PlayerInternalMeridian4})
    SL:AttachTXTSUI({root = PlayerInternalMeridian._ui["Panel_show_"..5], index = SLDefine.SUIComponentTable.PlayerInternalMeridian5})
end

function PlayerInternalMeridian.InitUIData()
    PlayerInternalMeridian._openList = SL:GetValue("MERIDIAN_OPEN_LIST")
...
function PlayerInternalMeridian.RefreshShowPanel()
    local function GetBgResPath()
        if PlayerInternalMeridian._selType == 2 then
            return string.format("m_bg1_%s.png", SL:GetValue("SEX"))
        end
        return string.format("m_bg0_%s.png", SL:GetValue("SEX"))
    end
    ...
    local lv = SL:GetValue("MERIDIAN_LV", real_typeID[PlayerInternalMeridian._selType])
    ...
    GUI:Image_loadTexture(PlayerInternalMeridian._ui.Image_m, resPath .. string.format("%s_%02d.png", title_strs[PlayerInternalMeridian._selType], math.min(lv, 5)))
    GUI:Image_loadTexture(PlayerInternalMeridian._ui.Image_l, resPath .. string.format("%s_xing.png", title_strs[PlayerInternalMeridian._selType]))
    GUI:Image_loadTexture(PlayerInternalMeridian._ui.Image_icon, resPath .. string.format("%s.png", PlayerInternalMeridian._selType))
    ...
    for i = 1, 5 do
        ...
        local openState = SL:GetValue("MERIDIAN_AUCPOINT_STATE", realID)
            ...
            if descList and descList[index] then
                ...
                if not isOpen then
                    data.btnShow = {
                        ...
                        btnName = "激活",
                        clickFunc = function()
                            SL:RequestMeridianAcuPointOpen(realID, i)
                        end
                    }
                end
...
function PlayerInternalMeridian.OnClose()
    ...
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalMeridian
    })
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalMeridian1
    })
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalMeridian2
    })
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalMeridian3
    })
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalMeridian4
    })
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalMeridian5
    })
end
