...
function PlayerInternalSkill.main()
    ...
    GUI:LoadExport(parent, isPC and "internal_player/internal_skill_node_win32" or "internal_player/internal_skill_node")
    ...
    SL:AttachTXTSUI({root = PlayerInternalSkill._ui["Panel_1"], index = SLDefine.SUIComponentTable.PlayerInternalSkill})
end
...
function PlayerInternalSkill.UpdateSkillCells()
    ...
    --已学的技能
    local items = SL:CopyData(SL:GetValue("INTERNAL_SKILLS"))
...
function PlayerInternalSkill.UpdateSkillCell(key)
    ...
    local skill =  SL:GetValue("INTERNAL_SKILL_DATA", skillID, skillType)
    if not skill then
        return
    end
    -- icon
    local contentSize = GUI:getContentSize(cell.Image_icon)
    local iconPath  = SL:GetValue("INTERNAL_SKILL_RECT_ICON_PATH", skillID, skillType) 
    ...
    -- 熟练度 等级
    local strTrain = SL:GetValue("INTERNAL_SKILL_TRAIN_DATA", skillID, skillType) 
    ...
    -- 开关显示
    local state = SL:GetValue("INTERNAL_SKILL_ONOFF", skillID, skillType)
...
function PlayerInternalSkill.CreateSkillCell(parent, skillID, skillType)
    GUI:LoadExport(parent, isPC and "internal_player/skill_cell_win32" or "internal_player/skill_cell")
    local ui = GUI:ui_delegate(parent)
    local cell = GUI:getChildByName(parent, "Panel_skill_cell")

    local name = SL:GetValue("INTERNAL_SKILL_NAME", skillID, skillType) 
    ...
    GUI:addOnClickEvent(ui.Button_onoff, function(sender)
        ...
        SL:SetValue("INTERNAL_SKILL_ONOFF", skillID, skillType, state)
    end)
    return cell, ui
end
...
function PlayerInternalSkill.OnClose()
    ...
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalSkill
    })
end
