...
local function getAttStr(title, key)
    local str = ""
    if key == 1 then
        str = string.format(title, SL:GetValue("CUR_ABIL_BY_ID", GUIDefine.AttTypeTable.Internal_LEVEL))
    elseif key == 2 then
        str = string.format(title, SL:GetValue("CUR_ABIL_BY_ID", GUIDefine.AttTypeTable.Internal_EXP))
    elseif key == 3 then
        str = string.format(title, SL:GetValue("MAX_ABIL_BY_ID", GUIDefine.AttTypeTable.Internal_EXP))
    elseif key == 4 then
        local value    = SL:GetValue("CUR_ABIL_BY_ID", GUIDefine.AttTypeTable.Internal_Value)
        local maxValue = SL:GetValue("MAX_ABIL_BY_ID", GUIDefine.AttTypeTable.Internal_Value)
...
local function getAttrShow(idx)
    local att = GUIFunction:GetAttDataShow({id = idx, value = SL:GetValue("CUR_ABIL_BY_ID", idx) or 0}, nil)
    att = att[idx] or {}
    local str = (att.name or "") .. " " .. (att.value or "")
    if idx == 105 and next(att) then -- 斗转星移值
        str = str .. string.format("/%s", SL:GetValue("MAX_ABIL_BY_ID", GUIDefine.AttTypeTable.Internal_DZValue))
    end
    return str
end
...
function PlayerInternalState.main()
    ...
    GUI:LoadExport(parent, isPC and "internal_player/internal_state_node_win32" or "internal_player/internal_state_node")
    ...
    SL:AttachTXTSUI({root = PlayerInternalState._ui["Panel_1"], index = SLDefine.SUIComponentTable.PlayerInternalState})
end
...
function PlayerInternalState.OnClose()
    ...
    SL:UnAttachTXTSUI({
        index = SLDefine.SUIComponentTable.PlayerInternalState
    })
end
