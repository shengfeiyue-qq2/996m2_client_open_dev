...
function LookPlayerSuperEquip.GetLooks(pos)
    ...
    -- 通过唯一ID MakeIndex 获取装备数据
    if pos == EquipPosCfg.Equip_Type_Super_Dress and equipData and LookPlayerSuperEquip._feature.showNodeModel and equipData.shonourSell and tonumber(equipData.shonourSell) == 1 then -- shonourSell == 1 不显示裸模, 服务器下发字段
...
function LookPlayerSuperEquip.InitEquipCells()
    ...
    if openFEquip and openFEquip == 0 then
        table.insert(LookPlayerSuperEquip._EquipPosSet, 14)
        table.insert(LookPlayerSuperEquip._EquipPosSet, 15)
