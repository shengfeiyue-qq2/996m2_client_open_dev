...
function PlayerSkill_Look_TradingBank.UpdateSkillCells()
    ...
    local items = SL:CopyData(TradingBankLookPlayerData.GetSkills())
...
function PlayerSkill_Look_TradingBank.UpdateSkillCell(skillID)
    ...
    local skill = TradingBankLookPlayerData.GetSkillByID(skillID)
    if not skill then
        return
    end
    -- icon
    ...
    --熟练度 等级
    local strTrain = TradingBankLookPlayerData.GetSkillTrainData(skillID)
...
function PlayerSkill_Look_TradingBank.OnClose()
end

PlayerSkill_Look_TradingBank.main()