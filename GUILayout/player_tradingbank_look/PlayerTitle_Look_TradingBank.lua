PlayerTitle_Look_TradingBank = {}
PlayerTitle_Look_TradingBank._ui = nil


PlayerTitle_Look_TradingBank._path = GUIDefine.PATH_RES_PRIVATE ..  "title_layer_ui/"

function PlayerTitle_Look_TradingBank.main()
    local parent = GUI:GetLayerOpenParam()
    GUI:SetLayerOpenParam(nil)
    GUI:LoadExport(parent, "player_look_tradingbank/player_title_node")
    ...
    PlayerTitle_Look_TradingBank.InitUI()
end

function PlayerTitle_Look_TradingBank.InitUI()
    -- 已激活的称号
    local activeID = TradingBankLookPlayerData.GetActiveTitle()

    PlayerTitle_Look_TradingBank.RefCurTitle(activeID)

    PlayerTitle_Look_TradingBank.RefTitleList(activeID)
end

-- 刷新当前穿戴
function PlayerTitle_Look_TradingBank.RefCurTitle(activeID)
    local btnCurTitle = PlayerTitle_Look_TradingBank._ui["Button_curTitle"]
    local lblCurTitle = PlayerTitle_Look_TradingBank._ui["Text_curTitle"]
...
-- 刷新称号列表
function PlayerTitle_Look_TradingBank.RefTitleList(activeID)
    -- 称号数据
    local titles = TradingBankLookPlayerData.GetTitle()
    ...
    local list = PlayerTitle_Look_TradingBank._ui["ListView_cells"]
    ...
    for i = 1, count do
        ...
        if titleList[i] then
            ...
            local res = PlayerTitle_Look_TradingBank._path .. "title_4.png"
            GUI:Button_loadTextureNormal(buttonIcon, res)
...
function PlayerTitle_Look_TradingBank.OnClose()
end

PlayerTitle_Look_TradingBank.main()