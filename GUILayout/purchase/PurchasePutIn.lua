...
function PurchasePutIn.main()
    local parent = GUI:Win_Create(UIConst.LAYERID.PurchasePutInGUI, 0, 0, 0, 0, false, false, true, true)
    GUI:LoadExport(parent, SL:GetValue("IS_PC_OPER_MODE") and "purchase_win32/purchase_putin" or "purchase/purchase_putin")
    ...
    PurchasePutIn._itemSizeHei  = SL:GetValue("IS_PC_OPER_MODE") and 32 or 42
    PurchasePutIn._currencyList = SL:GetValue("PURCHASE_CURRENCIES")
    ...
    GUI:addOnClickEvent(PurchasePutIn._ui.Button_add, function(sender)
...
function PurchasePutIn.SearchAllItemsByKeyWord(str, itemList)
    ...
    for i = 1, #itemList do
        local name = SL:GetValue("ITEM_NAME", itemList[i])
...
function PurchasePutIn.InitLeftGroup()
    local items = SL:GetValue("PURCHASE_FILTER_LIST")
...
function PurchasePutIn.UpdateFilter(noNeedLoad)
    local config = SL:GetValue("PURCHASE_MENU_CONFIG_BY_ID", PurchasePutIn._filterID)
    ...
    -- 组2
    local items = SL:GetValue("PURCHASE_FILTER_LIST")[config.firstlevel] or {}
...
function PurchasePutIn.UpdateItemList(searchStr)
    ...
    local itemList = SL:GetValue("PURCHASE_ITEM_LIST_BY_TYPE", config.firstlevel, config.secondlevel)
        ...
        if not PurchasePutIn._itemTblView then
            ...
            GUI:TableView_setCellCreateEvent(PurchasePutIn._itemTblView, function(parent, idx, ID)
                if ID == "TableView_item" then
                    ...
                    local name = SL:GetValue("ITEM_NAME", PurchasePutIn._itemList[idx])
...
function PurchasePutIn.UpadateParamPanel()
    local itemData = SL:GetValue("ITEM_DATA", PurchasePutIn._selItemID)
    -- 物品名
    local name = SL:GetValue("ITEM_NAME", PurchasePutIn._selItemID)
...
function PurchasePutIn.ChangeSelCurrency(name)
    ...
    name = name or SL:GetValue("ITEM_NAME", coinId)
    ...
    GUI:Text_setString(PurchasePutIn._ui.Text_coin_num, SL:GetValue("MONEY", coinId))
end

-- 左侧列表cell
function PurchasePutIn.CreateFilterGroupCell()
    local root = GUI:Node_Create(-1, "node", 0, 0)
    GUI:LoadExport(root, SL:GetValue("IS_PC_OPER_MODE") and "purchase_win32/purchase_filter_group1_cell" or "purchase/purchase_filter_group1_cell")
...
function PurchasePutIn.CreateFilterItemCell()
    local root = GUI:Node_Create(-1, "node", 0, 0)
    GUI:LoadExport(root, SL:GetValue("IS_PC_OPER_MODE") and "purchase_win32/purchase_filter_item_cell" or "purchase/purchase_filter_item_cell")
    local layout = GUI:getChildByName(root, "Panel_1")
    GUI:removeFromParent(layout)
    local ui = GUI:ui_delegate(layout)
