...
function PurchaseWorld.main()
    ...
    GUI:LoadExport(parent, SL:GetValue("IS_PC_OPER_MODE") and "purchase_win32/purchase_world" or "purchase/purchase_world")
    ...
    PurchaseWorld._currencies   = SL:GetValue("PURCHASE_CURRENCIES")
    table.insert(PurchaseWorld._currencies, 1, {id = 0, name = "全部"})


    -- 单个列表cell 尺寸
    PurchaseWorld._itemSize = SL:GetValue("IS_PC_OPER_MODE") and {width = 505, height = 70} or {width = 605, height = 80}
...
function PurchaseWorld.UpdateFilter1()
    -- 组1
    local config = SL:GetValue("PURCHASE_MENU_CONFIG_BY_ID", PurchaseWorld._filterID)
    ...
    -- 组2
    if PurchaseWorld._filter1State then
        local data = SL:GetValue("PURCHASE_FILTER_LIST")[config.firstlevel] or {}
        local items = {}
...
function PurchaseWorld.CreateFilterGroup2Cell()
    local root = GUI:Node_Create(-1, "node", 0, 0)
    GUI:LoadExport(root, SL:GetValue("IS_PC_OPER_MODE") and "purchase_win32/purchase_filter_group2_cell" or "purchase/purchase_filter_group2_cell")
    local layout = GUI:getChildByName(root, "Panel_1")
    GUI:removeFromParent(layout)
    local ui = GUI:ui_delegate(layout)
...
function PurchaseWorld.CreateItemCell(parent, item)
    GUI:LoadExport(parent, SL:GetValue("IS_PC_OPER_MODE") and "purchase_win32/purchase_world_cell" or "purchase/purchase_world_cell")
    ...
    GUI:addOnClickEvent(ui.Button_oper, function()
        UIOperator:OpenPurchaseSellUI(item)
    end)
...
function PurchaseWorld.CreatePriceCell(parent, data)
    GUI:LoadExport(parent, SL:GetValue("IS_PC_OPER_MODE") and "purchase_win32/purchase_price_cell" or "purchase/purchase_price_cell")
