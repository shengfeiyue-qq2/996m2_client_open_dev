    ...
    GUI:addOnClickEvent(Image_2, function()
        ...
        local func = function(res)
            if res ~= 0 then --选中了对应物品的编号  0是关闭
                SL:SetValue("SETTING_VALUE", data.id, {nil, nil, indexs[res] })
                --刷新UI
                refName(indexs[res])
            end
        end
        ...
        --打开 选则下拉框
        UIOperator:OpenCommonSelectListUI(items, position, size.width, size.heigth, func)
    end)
...
function SettingAuto.CreateClickInput2ListCell(parent, data)
    ...
    GUI:TextInput_addOnEvent(TextField_input2, function(_, eventType)
        if eventType == 1 then
            ...
            if count ~= "" then
                count = tonumber(count) or 0
                SL:SetValue("SETTING_VALUE", data.id, {nil, nil, count })
            end
        end
    end)
    ...
    GUI:addOnClickEvent(Image_2, function()
        ...
        local func = function(res)
            if res ~= 0 then --选中了对应物品的编号  0是关闭
                SL:SetValue("SETTING_VALUE", data.id, {nil, nil, nil, indexs[res] })
                --刷新UI
                refName(indexs[res])
            end
        end
...
function SettingAuto.CreateClickInpuCell(parent, data)
    ...
    GUI:addOnClickEvent(CheckBox_able, function()
        local values = SL:GetValue("SETTING_VALUE", data.id)
        local enable = values[1] == 1
        SL:SetValue("SETTING_VALUE", data.id, { enable and 0 or 1, values[2] or 8 })
        GUI:setVisible(Image_sel, not enable)
    end)
    return Panel_Layout
end
        ...
        or data.id == SLDefine.SETTINGID.SETTING_IDX_AUTO_SIMPSKILL then
            ...
            local func = function(res)
                if res ~= 0 then --0关闭  非0 选中的编号
                    if data.id == SLDefine.SETTINGID.SETTING_IDX_BEDAMAGED_PLAYER then
                        SL:SetValue("SETTING_VALUE", data.id, { res })
            ...
            UIOperator:OpenCommonSelectListUI(items, position, size.width, size.heigth, func)--打开选则列表
        end
    end)
...
function SettingAuto.OnAddIgnoreList(name)
    ...
    if Panel_UI then
        ...
        value[name] = 1
        SL:SetValue("SETTING_VALUE", SLDefine.SETTINGID.SETTING_IDX_IGNORE_MONSTER, { value })
    end
end
