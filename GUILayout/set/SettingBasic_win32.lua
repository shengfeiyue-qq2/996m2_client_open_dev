...
local group2 = {
    ...
    SLDefine.SETTINGID.SETTING_IDX_DISCARD_GOODS, --丢弃物品
}
...
function SettingBasic.main()
    ...
    --监听开关
    SL:RegisterLUAEvent(LUA_EVENT_SETTING_CHANGE, "SettingBasic", SettingBasic.OnSettingChange)
    
end

function SettingBasic.CloseLayer()
    SL:UnRegisterLUAEvent(LUA_EVENT_SETTING_CHANGE, "SettingBasic")
end
...
function SettingBasic.CreateVoiceProgressCell(parent, data)
    ...
    GUI:Slider_addOnEvent(Slider_progress, function(_, eventType)
        if eventType == 0 then
            ...
            local value = math.floor((SL:GetValue("SETTING_ENABLED", data.id) or 100) / 10) * 10
            local newValue = math.floor(percent / 10) * 10
            if math.abs(value - newValue) >= 10 then
                --设置新值
                SL:SetValue("SETTING_VALUE", data.id, { newValue })
                --刷新ui
