...
function SettingLaunch.CreateInputCell(parent, data)
    ...
    -- 描述
    local Text_desc = GUI:Text_Create(Panel_Layout, "Text_desc", 4, 18, 16, "#ffffff", data.content or "")
    GUI:setAnchorPoint(Text_desc, 0, 0.5)
    GUI:setTouchEnabled(Text_desc, false)

    -- 输入框背景
    local Image_2 = GUI:Image_Create(Panel_Layout, "Image_2", GUI:getContentSize(Text_desc).width + 50, 18, "res/private/new_setting/textBg.png")
    GUI:Image_setScale9Slice(Image_2, 33, 33, 9, 9)
    GUI:setContentSize(Image_2, 69, 28)
    GUI:setIgnoreContentAdaptWithSize(Image_2, false)
    GUI:setAnchorPoint(Image_2, 0.5, 0.5)
    ...
    GUI:Text_Create(Panel_Layout, "Text_per", GUI:getContentSize(Text_desc).width + 90, 8, 16, "#ffffff",  "%")

    -- 输入框
    local TextField_input = GUI:TextInput_Create(Image_2, "TextField_input", 4, 1, 60, 24, 16)
    GUI:TextInput_setTextHorizontalAlignment(TextField_input, 1)
