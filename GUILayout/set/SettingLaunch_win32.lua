...
SettingLaunch._isHaveHero = false
SettingLaunch._newAutoSummon = false --新的自动召唤 
...
function SettingLaunch.main()
    ...
    SettingLaunch._newAutoSummon = SL:GetValue("SERVER_OPTION", SW_KEY_NEW_AUTO_SUMMON)
...
function SettingLaunch.InitSkillGroup()
    ...
    -- 连击
    local comboSkills = SL:GetValue("SET_COMBO_SKILLS")
...
function SettingLaunch.CreateCell(parent, config)
    ...
    elseif config.id == SLDefine.SETTINGID.SETTING_IDX_FIRE_OPACITY then --火墙淡化
        cell = SettingLaunch.CreateInputCell(parent, config)
...
function SettingLaunch.CreateSelectInputCell(parent, data)
    ...
    GUI:TextInput_addOnEvent(TextField_input, function(_, eventType)
        if eventType == 1 then
            ...
            local timeSec = input
            SL:SetValue("SETTING_VALUE", data.id, {nil, timeSec })
        end
    end)
    ...
    if skillID and skillID ~= -1  then
        ...
        local skillItem = SettingLaunch.CreateSkillIcon(Image_skill, skillID)
...
function SettingLaunch.CreateSelectSkillCell(parent, data)
    ...
    if data.skill.MagicID == -1 then
        ...
        elseif data.setting.id == SLDefine.SETTINGID.SETTING_IDX_AUTO_SUMMON then
            RichText = GUI:RichText_Create(Panel_item, "RichText_auto", size.width / 2, size.height / 2, SettingLaunch._newAutoSummon and "智能召唤" or "无")
        end
        GUI:setAnchorPoint(RichText, 0.5, 0.5)
        GUI:setVisible(Image_skill, false)
    else
        local contentSize = GUI:getContentSize(Image_skill)
        local skillItem = SettingLaunch.CreateSkillIcon(Image_skill, data.skill.MagicID)
        GUI:setPosition(skillItem, contentSize.width / 2, contentSize.height / 2)
        GUI:setTouchEnabled(skillItem, false)
        GUI:setScale(skillItem, 0.9)
    ...
    GUI:addOnClickEvent(Panel_item, function()
        ...
        if data.setting.id == SLDefine.SETTINGID.SETTING_IDX_AUTO_LAUNCH then
            ...
            SL:SetValue("SETTING_VALUE", data.setting.id, { tgroupID })
        else
            tgroupID = data.skill.skillgroup
            SL:SetValue("SETTING_VALUE", data.setting.id, {nil, tgroupID })
        end
        ...
        if tgroupID  then
            if tgroupID == -1 then 
                ...
                elseif data.setting.id == SLDefine.SETTINGID.SETTING_IDX_AUTO_SUMMON then
                    GUI:Text_setString(Text_empty, SettingLaunch._newAutoSummon and "智能召唤" or "无")
                end
                GUI:setVisible(Text_empty, true)
            else
                local contentSize = GUI:getContentSize(Image_skill)
                local skillItem = SettingLaunch.CreateSkillIcon(Image_skill, tgroupID)
                GUI:setPosition(skillItem, contentSize.width / 2, contentSize.height / 2)
                GUI:setTouchEnabled(skillItem, false)
                GUI:setScale(skillItem, 0.9)
                GUI:setVisible(Text_empty, false)
...
--创建 输入
function SettingLaunch.CreateInputCell(parent, data)
    local values = SL:GetValue("SETTING_VALUE", data.id)
    -- 容器
    local Panel_Layout = GUI:Layout_Create(parent, "Panel_" .. data.id, 0, 2, 200, 40, false)
    GUI:setTouchEnabled(Panel_Layout, true)
    ...
    -- 描述
    local Text_desc = GUI:Text_Create(Panel_Layout, "Text_desc", 4, 18, 13, "#ffffff", data.content or "")
    GUI:setAnchorPoint(Text_desc, 0, 0.5)
    GUI:setTouchEnabled(Text_desc, false)

    -- 输入框背景
    local Image_2 = GUI:Image_Create(Panel_Layout, "Image_2", GUI:getContentSize(Text_desc).width + 40, 18, "res/private/new_setting/textBg.png")
    GUI:Image_setScale9Slice(Image_2, 33, 33, 9, 9)
    GUI:setContentSize(Image_2, 50, 28)
    GUI:setIgnoreContentAdaptWithSize(Image_2, false)
    GUI:setAnchorPoint(Image_2, 0.5, 0.5)
    ...
    GUI:Text_Create(Panel_Layout, "Text_per", GUI:getContentSize(Text_desc).width + 70, 8, 13, "#ffffff",  "%")

    --输入框
    local TextField_input = GUI:TextInput_Create(Image_2, "TextField_input", 1, 1, 47, 24, 12)

    GUI:TextInput_setTextHorizontalAlignment(TextField_input, 1)
    GUI:TextInput_setMaxLength(TextField_input, 10)
    GUI:TextInput_setInputMode(TextField_input, 2)

    local value = values[2] and values[2] or 100
    GUI:TextInput_setString(TextField_input, value)

    GUI:TextInput_addOnEvent(TextField_input, function(_, eventType)
        if eventType == 1 then
            local str = GUI:TextInput_getString(TextField_input)
            local num = math.min(tonumber(str) or 100, 100)
            SL:SetValue("SETTING_VALUE", data.id, {nil, math.max(num or 100, 1)})
        elseif eventType == 3 then
            local str = GUI:TextInput_getString(TextField_input)
            local num = math.min(tonumber(str) or 100, 100)
            GUI:TextInput_setString(TextField_input, num or 100)
        end
    end)
...
function SettingLaunch.CreateSelectColorCell(parent, data)
    -- 容器
    local Panel_Layout = GUI:Layout_Create(parent, "Panel_" .. data.id, 0, 0, 190, 40, false)
    GUI:setTouchEnabled(Panel_Layout, true)
    ...
    -- 点击的背景
    local Image_Click = GUI:Image_Create(Panel_Layout, "Image_Click", 90, 8, "res/private/new_setting/textBg.png")
    GUI:Image_setScale9Slice(Image_Click, 33, 33, 9, 9)
    GUI:setContentSize(Image_Click, 74, 28)
    GUI:setIgnoreContentAdaptWithSize(Image_Click, false)
    GUI:setTouchEnabled(Image_Click, true)

    -- 描述
    local Text_desc2 = GUI:Text_Create(Image_Click, "Text_desc2", 35, 14, 12, "#FFFFFF", "")
    GUI:setAnchorPoint(Text_desc2, 0.5, 0.5)
    GUI:setTouchEnabled(Text_desc2, false)
    GUI:Text_enableOutline(Text_desc2, "#111111", 1)

    -- 颜色显示
    local Layout_color = GUI:Layout_Create(Image_Click, "Layout_color", 2, 2, 70, 24)
    ...
    GUI:addOnClickEvent(Image_Click, function()
        ...
        UIOperator:OpenCommonSelectListUI(colorItems, position, size.width, size.heigth, func, {isColorShow = true, fontSize = 12}) -- 打开选择列表
    end)
