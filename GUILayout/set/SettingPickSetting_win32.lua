...
function SettingPickSetting.main()
    if SettingPickSetting._layer then 
        return 
    end
    SettingPickSetting._layer = GUI:Win_Create(UIConst.LAYERID.SettingPickSetGUI, 0, 0, 0, 0, false, false, true, true)
    GUI:LoadExport(SettingPickSetting._layer, "set/setting_pick_setting_win32")
    SettingPickSetting._ui = GUI:ui_delegate(SettingPickSetting._layer)
    local Panel_1 = SettingPickSetting._ui["Panel_1"]
    local screenW = SL:GetValue("SCREEN_WIDTH")
    local screenH = SL:GetValue("SCREEN_HEIGHT")
    GUI:setPosition(Panel_1, screenW / 2, SL:GetValue("PC_POS_Y"))
    if not SettingPickSetting._ui then
        return false
    end
    SettingPickSetting._items = {}
    SettingPickSetting._groupCells = {}
    SettingPickSetting.InitFrameUI()
    SettingPickSetting.Init()
    SettingPickSetting.RegisterEvent()
end

--外框
function SettingPickSetting.InitFrameUI()
...
--内容
function SettingPickSetting.Init()
    local node = SettingPickSetting._ui.Node_1
...
function SettingPickSetting.InitItemConfig()
    local itemConfig = SL:GetValue("STD_ITEMS")
    local groupConfig = SL:GetValue("SETTING_PICK_CONFIG")
...
function SettingPickSetting.CreateGroupCell(parent, data)
    ...
    local jpPick = false
    --这里当总开关  不管里面有没有都能勾
    local value =   SL:GetValue("SETTING_PICK_GROUP_VALUE", data.group) 
    drop = value[1] and value[1] == 1
    pick = value[2] and value[2] == 1
    autopick = value[3] and value[3] == 1
    jpPick = value[4] and value[4] == 1
    -- 
    ...
    GUI:addOnClickEvent(ui.Panel_drop,function()
        local groupvalue = SL:GetValue("SETTING_PICK_GROUP_VALUE", data.group) 
        groupvalue[1] = groupvalue[1] == 0 and 1 or 0
        local isSelected = groupvalue[1] == 1
        SL:SetValue("SETTING_PICK_GROUP_VALUE", data.group, groupvalue)
        for _, v in pairs(items) do
            local value = SL:GetValue("SETTING_PICK_VALUE", v.Index)
            value[1]    = isSelected and 1 or 0
            SL:SetValue("SETTING_PICK_VALUE", v.Index, value)
        end
    ...
    -- 极品拾取
    GUI:CheckBox_setSelected(ui.CheckBox_pick_jp, jpPick)
    GUI:addOnClickEvent(ui.Panel_pick_jp, function()
        local groupvalue = SL:GetValue("SETTING_PICK_GROUP_VALUE", data.group) 
        groupvalue[4] = groupvalue[4] == 0 and 1 or 0
        local isSelected = groupvalue[4] == 1
        
        SL:SetValue("SETTING_PICK_GROUP_VALUE", data.group, groupvalue)
        for _, v in pairs(items) do
            local value = SL:GetValue("SETTING_PICK_VALUE", v.Index)
            value[4]    = isSelected and 1 or 0
            SL:SetValue("SETTING_PICK_VALUE", v.Index, value)
        end
        GUI:CheckBox_setSelected(ui.CheckBox_pick_jp, groupvalue[4] == 1)
    end)
    -- 
    GUI:Text_enableUnderline(ui.Text_desc)
...
function SettingPickSetting.CreatePickCell(parent, data)
    ...
    GUI:addOnClickEvent(ui.Panel_drop, function()
        ...
        SL:SetValue("SETTING_PICK_VALUE", data.Index, value)
    ...
    GUI:addOnClickEvent(ui.Panel_pick_jp, function()
        local value = SL:GetValue("SETTING_PICK_VALUE", data.Index)
        value[4]    = value[4] == 0 and 1 or 0
        local isSelect = value[4] == 1
        SL:SetValue("SETTING_PICK_VALUE", data.Index, value)
        GUI:CheckBox_setSelected(ui.CheckBox_pick_jp, isSelect)
    end)
    ...
    GUI:setVisible(ui.Panel_pick_jp, cansets[4] == 1)
...
function SettingPickSetting.RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_CLOSEWIN, "SettingPickSetting", SettingPickSetting.OnCloseLayer) --关闭界面
end
