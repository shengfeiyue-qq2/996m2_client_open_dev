...
function SettingProtectSetting.main()
    local data = GUI:GetLayerOpenParam()
    local parent = GUI:Win_Create(UIConst.LAYERID.SettingProtectSetGUI, 0, 0, 0, 0, false, false, true, true)
...
function SettingProtectSetting.InitUI(data)
    ...
    GUI:TextInput_addOnEvent(ui.TextField_input, function(_, eventType)
        if eventType == 1 then
            ...
            if time ~= "" then
                ...
                local config = SL:GetValue("SETTING_CONFIG", data.id)
    ...
    SettingProtectSetting._rankData = SL:GetValue("SETTING_RANK_DATA", SettingProtectSetting._protectDataId)
        ...
        elseif type == GUIDefine.TouchEventType.MOVED then
            if SettingProtectSetting._selItem then
                ...
                if not SettingProtectSetting._touchItem then
                    ...
                    local name = SL:GetValue("ITEM_NAME", index)--道具名字
...
function SettingProtectSetting.SaveSet()
    SL:SetValue("SETTING_RANK_DATA", SettingProtectSetting._protectDataId, SettingProtectSetting._rankData)
end

SettingProtectSetting.main()