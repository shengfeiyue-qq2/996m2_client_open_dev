...
function SettingSkillRank.main()
    local data = GUI:GetLayerOpenParam()
    local parent = GUI:Win_Create(UIConst.LAYERID.SettingSkillRankGUI, 0, 0, 0, 0, false, false, true, true)
...
function SettingSkillRank.InitUI(data)
    ...
    SettingSkillRank._rankData = SL:GetValue("SETTING_RANK_DATA", SettingSkillRank._protectDataId)
...
function SettingSkillRank.AddItem(order, index)
    ...
    local name = SL:GetValue("SKILL_NAME", index)--技能名称
...
function SettingSkillRank.SaveSet()
    SL:SetValue("SETTING_RANK_DATA", SettingSkillRank._protectDataId, SettingSkillRank._rankData)
end

function SettingSkillRank.OnCloseLayer(id)
    if id == UIConst.LAYERID.SettingSkillRankGUI then 
...
function SettingSkillRank.RegisterEvent()
    ...
    SL:RegisterLUAEvent(LUA_EVENT_CLOSEWIN, "SettingSkillRank", SettingSkillRank.OnCloseLayer) --关闭界面
end

function SettingSkillRank.UnRegisterEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_SKILL_RANKDATA_ADD, "SettingSkillRank")
    SL:UnRegisterLUAEvent(LUA_EVENT_CLOSEWIN, "SettingSkillRank")
end

SettingSkillRank.main()