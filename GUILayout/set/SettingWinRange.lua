...
function SettingWinRange.CreateProgressCell(parent, data)
    ...
    GUI:Slider_setPercent(Slider_progress, SL:GetValue("SETTING_MAPSCALE_PER", value)) -- 地图缩放百分比

    GUI:Slider_addOnEvent(Slider_progress, function(_, eventType)
        if eventType == 0 then
            local percent = GUI:Slider_getPercent(Slider_progress)
            local value    = SL:GetValue("SETTING_ENABLED", (data.id))
            local newValue = math.floor(SL:GetValue("SETTING_MAPSCALE_VALUE", percent) * 10) / 10
