...
function StoreDetail.main()
    local parent = GUI:Win_Create(UIConst.LAYERID.StoreDetailGUI, 0, 0, 0, 0, false, false, true, true)
...
function StoreDetail.InitBuyBtn()
    local function buy()
        local data = StoreDetail.buyData
        if data.Condition and not SL:CheckCondition(data.Condition) then
            return
        end

        local bagSpace = true -- 背包是否有格子
        if StoreDetail.buyCount > 0 then
            bagSpace = BagData.CheckNeedSpace(data.ID, StoreDetail.buyCount, true)
        end
...
function StoreDetail.InitGoodsItem(data)
    ...
    if name then
        GUI:Text_setString(StoreDetail._ui["Text_itemName"], name)
...
function StoreDetail.GetTotalMoney(data, totalPrice)
    ...
    if data.ArrConstID and next(data.ArrConstID) then
        ...
        for i, id in ipairs(data.ArrConstID) do
            local count= tonumber(SL:GetValue("MONEY", id)) 
        ...
        local count = tonumber(SL:GetValue("MONEY", data.CostType))  
        if data.CostType == MoneyType.BindYuanBao and isBind then 
            count = count + tonumber(SL:GetValue("MONEY", MoneyType.YuanBao))
        end 

        if count >= totalPrice then 
            isCanBuy = true
        end 
        table.insert(moneys, {id = data.CostType, count = totalPrice}) 
    end
    return moneys, isCanBuy
end
...
function StoreDetail.GetHaveMoney(data, totalPrice)
    ...
    if data.ArrConstID and next(data.ArrConstID) then
        ...
        table.insert(moneys, {id = data.CostType, count = count}) 
    end
    return moneys
end

function StoreDetail.OnCloseLayer()
    UIOperator:CloseStoreDetailUI()
end

function StoreDetail.OnClose(UID)
    if UID ~= UIConst.LAYERID.StoreDetailGUI then
        return false
    end

    StoreDetail.UnRegisterEvent()
end

function StoreDetail.RegisterEvent()
    SL:RegisterLUAEvent(LUA_EVENT_CLOSEWIN, "StoreDetail", StoreDetail.OnClose)
    SL:RegisterLUAEvent(LUA_EVENT_STORE_BUY_CLOSE, "StoreDetail", StoreDetail.OnCloseLayer)
end

function StoreDetail.UnRegisterEvent()
    SL:UnRegisterLUAEvent(LUA_EVENT_CLOSEWIN, "StoreDetail")
    SL:UnRegisterLUAEvent(LUA_EVENT_STORE_BUY_CLOSE, "StoreDetail")
end
